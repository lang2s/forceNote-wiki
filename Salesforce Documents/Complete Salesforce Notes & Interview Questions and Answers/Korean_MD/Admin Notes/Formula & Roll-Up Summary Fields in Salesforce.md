...
**특징:** 값을 동적으로 계산(참조 필드 변경 시 자동 업데이트), 오브젝트 간 작동(관련 오브젝트 필드 참조 가능), 논리·수학 함수 지원(계산·텍스트 연결·IF 문), 사용자 편집 불가(읽기 전용).
...
**특징:** 자식 레코드 데이터 집계(Master-Detail 관계), SUM·COUNT·MIN·MAX 계산, 자식 레코드 변경 시 자동 업데이트, 오브젝트 간 참조 불가(같은 관계 내에서만).
