...
**Global Picklist Value Set:** 중복 규칙에서 지원되지 않습니다.

**Custom Picklist:** 오브젝트 간 중복 규칙에 사용되는 매칭 규칙에서 커스텀 선택 목록 필드는 지원되지 않습니다.

**Rollup Summary 필드 값 변경:** 롤업 요약 필드 값이 변경되면 중복 규칙이 실행되며, Allow 옵션(중복 레코드 저장)은 지원되지 않습니다.
