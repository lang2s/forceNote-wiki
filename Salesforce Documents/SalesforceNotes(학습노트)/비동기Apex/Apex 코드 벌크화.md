...
대량 데이터를 효과적으로 처리. **방법:** List·Set·Map 컬렉션 사용.
```apex
// 잘못된 예: 단일 레코드 처리
...
거버너 한도 초과 방지. **방법:** 루프 밖에서 쿼리, 컬렉션 사용.
```apex
// 잘못된 예: 루프 안 SOQL
...
코드를 동적·유지보수 용이하게. **방법:** 하드코딩 값을 구성 가능한 메타데이터로.
```apex
// 잘못된 예: 하드코딩
...
신뢰성 보장, 배포 오류 방지. **방법:** 벌크·긍정·부정·거버너 한도 케이스 테스트.
```apex
@IsTest
...
런타임 오류 방지, 의미 있는 피드백. **방법:** try-catch.
```apex
try {
...
코드 구조·재사용성 개선. **예: 유틸리티 클래스의 Singleton 패턴**
```apex
...
데이터 보안·조직 공유 설정 준수. **방법:** with/without sharing 적절히 사용.
```apex
