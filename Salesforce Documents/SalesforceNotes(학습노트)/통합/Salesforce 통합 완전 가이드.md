...
Salesforce를 외부 앱(DB·ERP·결제·CRM)과 연결해 데이터 교환.
**필요성:**

외부 DB(MySQL·PostgreSQL) 동기화, 마케팅 도구(HubSpot·Marketo) 연결, ERP(SAP) 주문 자동화, webhook·이벤트 기반 실시간 통신.
**유형:**

Data Integration(데이터 동기화), Process Integration(워크플로우 자동화), Security Integration(SSO).

## Part 2: API
**SOAP API:**

XML, 레거시(SAP·Oracle·은행), WSDL.
```xml
...
외부 앱 안전 연결. App Manager → New Connected App → OAuth 활성화·콜백 URL → 스코프 → Client ID·Secret.
**OAuth 2.0 방법:**

Username-Password(비프로덕션), JWT(서버 간), Web Server(사용자 인증).
```
