...
웹 표준, 최신 JS, @wire로 데이터 접근 단순화, Shadow DOM, 렌더링 최적화·보안·캐싱.
...
소문자 시작, 영숫자·언더스코어만. 공백·끝 언더스코어·연속 언더스코어·하이픈 불가.
...
owner가 속성 설정. 데이터 바인딩은 owner→child 단방향(Aura와 달리). 자식은 읽기 전용 취급, 변경하려면 부모에 이벤트.
```html
<!-- 부모 todoapp.html -->
...
`this.template.querySelector()`(첫 요소), `querySelectorAll()`(배열).
**Static Resource:**

`import myResource from '@salesforce/resourceUrl/...'`
**Label:**

`import labelName from '@salesforce/label/...'`
**Current UserId:**
...
컴포넌트·wire 어댑터·JS 함수. 레코드 캐시·공유, 서버 호출 최적화.
**기본 컴포넌트:**

lightning-record-form, lightning-record-edit-form, lightning-record-view-form. 생성/업데이트는 lightning/uiRecordApi(CRUD·FLS·공유 준수).
