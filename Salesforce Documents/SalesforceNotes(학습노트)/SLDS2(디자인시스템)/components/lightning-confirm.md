...
`lightning-confirm`  ·  카테고리: **Status & Notification**

확인/취소를 묻는 모달(프로그래밍 호출).
...
`lightning-confirm` 의 상세 사용법, 변형, 접근성(ARIA), 스타일링 훅 등 전체 설명은 공식 **Develop** 문서를 참고하세요. 이 컴포넌트는 SLDS 블루프린트를 구현하며, 조직 테마에 따라 SLDS 1 또는 SLDS 2 스타일로 렌더링됩니다.
