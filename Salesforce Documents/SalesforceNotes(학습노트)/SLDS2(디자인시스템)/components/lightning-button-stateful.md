...
`lightning-button-stateful`  ·  카테고리: **Action & Menu**

선택/비선택 상태에 따라 라벨·아이콘이 바뀌는 버튼. 예: 팔로우.

## 기본 예제 (Example)

```html
<lightning-button-stateful label-when-off="팔로우" label-when-on="팔로잉" selected={following} onclick={toggle}></lightning-button-stateful>
```
...
`lightning-button-stateful` 의 상세 사용법, 변형, 접근성(ARIA), 스타일링 훅 등 전체 설명은 공식 **Develop** 문서를 참고하세요. 이 컴포넌트는 SLDS 블루프린트를 구현하며, 조직 테마에 따라 SLDS 1 또는 SLDS 2 스타일로 렌더링됩니다.
