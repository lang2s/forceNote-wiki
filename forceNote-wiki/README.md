...
| [dreamhouse-lwc](https://github.com/trailheadapps/dreamhouse-lwc) | LWC 실제 앱, Permission Set, CSP/RemoteSite, 모바일, Static Resource, 파일 업로드, PagedResult | `LWC/Mobile(모바일)/`, `LWC/UIPatterns(UI패턴)/` 3개, `Apex/Data(데이터)/`, `Apex/Architecture(아키텍처)/`, `Integration(통합)/` 신규 노트 | `package-config`, `ci-workflow` |
...
| `api_asynch.pdf` | `Apex/Async(비동기)/` — Bulk API 한도, Job 상태 |
...
├── 00 Home.md              ← 전체 진입점
├── Apex/                   ← 36개 노트
│   ├── Security(보안)/         Safely, CanTheUser, WITH USER_MODE, StripInaccessible
│   ├── Async(비동기)/          Future, Queueable, Batch, Scheduled, 선택 매트릭스
│   ├── Data(데이터)/           SOQL, DML, Dynamic SOQL, PagedResult
│   ├── Integration(통합)/      RestClient, Custom REST Endpoint
│   ├── Testing(테스트)/        StubProvider, HttpCalloutMock, testVisible, SOSL
│   ├── Architecture(아키텍처)/ 서비스 레이어, Permission Set 설계
...
│   ├── PlatformEvents(플랫폼이벤트)/ Platform Event 발행
│   └── PlatformCache(플랫폼캐시)/   Platform Cache
├── LWC/                    ← 18개 노트
│   ├── ApexIntegration(Apex통합)/  Wire vs Imperative, @wire, async/await
│   ├── ComponentAPI(컴포넌트API)/  @api, 컴포지션 패턴
│   ├── Events(이벤트)/            CustomEvent, LMS, 상태 관리
│   ├── LDS/                       Record Form, uiRecordApi, getRecord, reduceErrors
│   ├── Navigation(네비게이션)/    NavigationMixin
│   ├── UIPatterns(UI패턴)/        Toast, 모달, 에러 패널, 공유 JS 모듈, Static Resource, 파일 업로드
...
└── Integration(통합)/      ← 4개 노트
    ├── Named Credential    External Credential, callout: 접두어
    ├── CSP와 RemoteSite    CspTrustedSite, RemoteSiteSetting 결정 매트릭스
