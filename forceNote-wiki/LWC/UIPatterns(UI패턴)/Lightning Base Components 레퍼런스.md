...
> **각 컴포넌트 상세 레퍼런스** (속성·이벤트·코드 예시·접근성·전체 공식 명세 포함) → 본 페이지 하단 **[[#상세 레퍼런스 페이지 (개별 컴포넌트 66개)|상세 레퍼런스 페이지 (66개)]]** 섹션 또는 [[BaseComponents(베이스컴포넌트)/index|BaseComponents 색인]] 참조.
...
## 상세 레퍼런스 페이지 (개별 컴포넌트 66개)

> 각 컴포넌트의 속성·이벤트·코드 예시·접근성·**전체 공식 속성 명세**(cx-router Tier 2)는 아래 개별 노트 참조. 폴더: `LWC/BaseComponents(베이스컴포넌트)/`
### 핵심 패밀리 (Tier 1 예제 + 전체 명세)
- [[lightning-accordion]] — 접고 펼 수 있는 아코디언
- [[lightning-tabset]] — 탭 그룹 컨테이너
- [[lightning-input]] — 다목적 입력 필드(type 14종)
- [[lightning-combobox]] — 단일 선택 드롭다운
- [[lightning-datatable]] — 정렬·선택·인라인편집 테이블
- [[lightning-modal]] — 모달 다이얼로그(+Alert/Confirm/Prompt)
- [[lightning-record-form]] — 레코드 폼 3종(+input/output-field)
- [[lightning-record-picker]] — 레코드 검색·선택 위젯
- [[lightning-button]] — 버튼 패밀리(icon/menu/group/stateful)
- [[lightning-card]] — 카드 컨테이너
- [[lightning-spinner]] — 로딩 스피너
