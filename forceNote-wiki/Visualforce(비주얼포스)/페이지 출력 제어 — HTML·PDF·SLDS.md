...
> ⚠️ **테스트 제약 — `getContent()` / `getContentAsPDF()`는 test 메서드에서 실행할 수 없다.** API v34.0 이상에서 두 메서드는 callout으로 취급되므로, `@isTest` 메서드 안에서 호출하면 **test 메서드가 실패**한다. 따라서 위 `PdfEmailerController` 같은 PDF 생성 컨트롤러를 단위 테스트할 때는 `getContentAsPDF()` 호출 경로를 직접 실행하지 말고, PDF 생성부를 별도 메서드로 분리해 mock/우회하거나 그 부분을 assert 대상에서 제외한다. 상세는 [[Apex 버전별 동작 변경 레퍼런스]] 참조. (커스텀 컨트롤러 테스트 idiom 전반은 [[커스텀 컨트롤러·컨트롤러 확장]] §10)
