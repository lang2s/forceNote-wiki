...
| Custom Metadata Type vs Custom Setting, CMDT vs Custom Setting, CMDT vs CS, 커스텀 메타데이터 vs 커스텀 설정, 구성 데이터 어디 담나, 설정값 어디 저장, 배포 가능 설정, deployable config, Hierarchy Custom Setting, 계층 커스텀 설정, getAll SOQL 면제, 앱 구성값 저장, 어느 것을 선택, 결정 가이드 | `Architecture(아키텍처)/Custom Metadata Type vs Custom Setting 결정 가이드.md` |
...
| Governor Limits, 거버너 한도, 실행 한도, SOQL 한도, DML 한도, Heap size, CPU time, Callout 한도, Limits 클래스, getQueries, getDmlStatements, getLimitQueries, getLimitDmlStatements, 거버너 리밋, Apex 실행 한도, 한도 초과 예외, LimitException, Per-Transaction Limits, 비동기 동기 한도 차이, Platform Apex Limits, Static Apex Limits, Apex heap 10MB, heap 25MB, Winter '27 heap 인상, Enforce the Summer '26 Apex heap limit, 구 heap 한도 되돌리기, Limits.getLimitHeapSize, Elastic Limits, elastic limits beta, Batch 잡 elastic limits, DailyAsyncApexElasticExecutions, 비동기 Apex 잡 한도 override, asyncApexExecutionsOverride, 비프로덕션 override | `Apex/ExecutionContext(실행컨텍스트)/Governor Limits.md` |
