...
# SEARCH INDEX — 프론트엔드 (LWC / Flow)
> LWC·Flow·Base Components(카탈로그) 키워드 → 파일
> 루트 라우터: `00 SEARCH_INDEX.md` · Aura → `_index/frontend-aura.md` · SLDS → `_index/frontend-slds.md` · 개별 lightning-* → `_index/frontend-basecomponents.md`
...
> **Aura 컴포넌트**(`Aura(오라)/`)는 별도 샤드로 분할됨 → `_index/frontend-aura.md` 참조.
...
> **SLDS / 디자인 시스템 · SLDS 2 Starter Kit**(`LWC/SLDS(디자인시스템)/` + SLDS 관련 UIPatterns)는 별도 샤드로 분할됨 → `_index/frontend-slds.md` 참조.
