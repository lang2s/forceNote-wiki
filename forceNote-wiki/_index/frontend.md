...
| @AuraEnabled, cacheable, component.get c.method, action.setCallback, $A.enqueueAction, action state, SUCCESS ERROR INCOMPLETE ABORTED, setStorable, storable action, 왜 옛 데이터가 오나, setBackground, getError, AuraHandledException, Continuation, force:recordData, Lightning Data Service, Aura LDS, 선언적 레코드 CRUD, recordUpdated, Aura 서버 액션, Aura Apex 연동, Aura에서 Apex 호출 | `Aura(오라)/Aura 데이터 연동 — 서버 Apex 액션 · Lightning Data Service.md` |
