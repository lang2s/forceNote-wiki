...
| 키워드 | 파일 |
|---|---|
| Agentforce 개요, Agentforce란 무엇, Agentforce가 뭐야, 에이전트 유형, agent types, Agentforce Service Agent, Sales Agent, Atlas Reasoning Engine, 아틀라스 추론 엔진, 추론 엔진, building blocks, 구성요소, Subagents, Topics, Actions, Agentforce Studio, Agentforce Builder, Prompt Builder vs Agent, 프롬프트 빌더와 차이, 언제 에이전트 vs Flow, 에이전트 언제 써, Einstein Copilot, 코파일럿, 제품 개요, 오리엔테이션 | `Agentforce(에이전트포스)/Agentforce 개요 — 제품·에이전트 유형·구성요소.md` |
...
## Agentforce — 커스텀 UI · 프롬프트 액션 → 파일

| 키워드 | 파일 |
|---|---|
| Custom Lightning Types, CLT, lightning__AgentforceInput, lightning__AgentforceOutput, targetType, sourceType, Lightning Type Bundle, schema.json, editor.json, renderer.json, lightningDesktopGenAi, componentOverrides, @apexClassType, complex_data_type_name, is_user_input, is_displayable, valuechange, 커스텀 라이트닝 타입, 에이전트 액션 입력 폼, 에이전트 출력 카드, 에디터 렌더러 LWC, Agentforce 커스텀 UI, 에이전트 액션에 커스텀 LWC UI 붙이기, Agentforce 입력을 폼으로 받는 법, 액션 결과를 카드로 표시 | `Agentforce(에이전트포스)/Agentforce 커스텀 Lightning Type — 에이전트 액션 입출력 UI.md` |
| prompt template action, generatePromptResponse, GenAiPromptTemplate, templateVersions, templateDataProviders, Apex data provider, InvocableMethod grounding, promptResponse output, Input reference name, flex template, 프롬프트 템플릿 액션, Apex 그라운딩, 프롬프트 데이터 프로바이더, 머지필드, 에이전트에서 프롬프트 템플릿 실행하는 법, 프롬프트 템플릿을 Apex로 그라운딩하려면, genAiPromptTemplate 메타 필드가 뭐야 | `Agentforce(에이전트포스)/Agentforce Prompt Template 액션 — genAiPromptTemplate·Apex 그라운딩.md` |
