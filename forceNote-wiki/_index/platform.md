...
| approval limits, approval history report, respond to approval, Mass Transfer Approval Requests, ProcessInstance 필드, Approvals in Chatter/Slack, 승인 한도, 승인 이력 리포트, 승인 응답 채널, 엔드유저 승인, 승인 프로세스 한도?, 승인 이력을 리포트로?, 이메일/Chatter/Slack로 승인?, 승인 요청 대량 이전? | `Admin(어드민)/Approval Process — 운영·엔드유저·레퍼런스.md` |
