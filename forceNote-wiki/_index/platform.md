...
| API Request Limits, Total API Allocations, Concurrent API Limits, Bulk API limits, SOAP call limits, Metadata API limits, SOQL search limits, Visualforce limits, view state, 일일 API 콜 한도, 동시 API 한도, 벌크 한도, 메타데이터 한도, SOQL 검색 한도, VF 한도, view state 한도, org 한도 어디서 봐, "API 콜 한도 얼마야", "동시 API 호출 제한", "Bulk API 배치 한도", "VF view state 한도", salesforce_app_limits_cheatsheet, Developer Limits and Allocations Quick Reference, 치트시트 판 대조, August 14 2026 판, May 8 2026 판, Apex 거버너 한도 표 치트시트에서 삭제, Push Notification 한도 구판, "치트시트에서 사라진 한도는 바뀐 건가" | `Architecture(아키텍처)/Salesforce 한도·할당량 레퍼런스 (API·Bulk·Metadata·SOQL·VF).md` |
| Salesforce 네비게이션, App Launcher, 앱 런처, 전역 검색, Global Search, 탐색바, 리스트뷰, 레코드 페이지, Lightning Experience, 홈 페이지 | `Admin(어드민)/OrgSetup(조직설정)/Salesforce 네비게이션.md` |
| Data Loader, 데이터 로더, 대량 적재, CSV 임포트, bulk import export, insert update upsert delete, hard delete, Bulk API 2.0, process-conf.xml, CLI 배치, Data Import Wizard 비교, 1.5억 건 | `Admin(어드민)/Data(데이터관리)/Data Loader.md` |
| Data Import Wizard, 데이터 임포트 마법사, CSV 임포트, import records, 데이터 가져오기, 50000 레코드 한도, 어떤 객체 임포트 가능, Launch Wizard, 마법사로 데이터 올리기 | `Admin(어드민)/Data(데이터관리)/Data Import Wizard.md` |
| Approval Process, Classic Approval Process, approval workflow, Jump Start Wizard, Standard Wizard, approval steps, assigned approver, delegated approver, record locking, outbound message, 승인 프로세스, 승인 워크플로, 승인 단계, 위임 승인자, 레코드 잠금, 승인 프로세스 만드는 법, Flow Approval Processes | `Admin(어드민)/Automation(자동화)/Approval Process (승인 프로세스).md` |
| approval limits, approval history report, respond to approval, Mass Transfer Approval Requests, ProcessInstance 필드, Approvals in Chatter/Slack, 승인 한도, 승인 이력 리포트, 승인 응답 채널, 엔드유저 승인, 승인 프로세스 한도?, 승인 이력을 리포트로?, 이메일/Chatter/Slack로 승인?, 승인 요청 대량 이전? | `Admin(어드민)/Automation(자동화)/Approval Process — 운영·엔드유저·레퍼런스.md` |
| MFA, Multi-Factor Authentication, 다중 인증, Salesforce Authenticator, TOTP, 보안 키, FIDO2, Trusted IP Ranges, 신원 확인, 이중 인증, MFA 의무화 | `Admin(어드민)/LoginSecurity(로그인·세션보안)/Salesforce ID 인증.md` |
| AddressSettings, CountriesAndStates, State and Country Picklist, State and Country/Territory Picklists, `Settings:Address`, Address.settings, Country State isoCode integrationValue, 국가/주 피클리스트, 국가 영토 피클리스트, 주소 설정, 국가 주 활성화, 주 도 피클리스트, "국가/주 피클리스트를 메타데이터로 설정하려면?", "AddressSettings로 국가 주 값 편집", 텍스트 주소를 표준 피클리스트로 변환 | `Admin(어드민)/OrgSetup(조직설정)/State and Country Picklist.md` |
| Formula Field, 수식 필드, 포뮬러 필드, cross-object formula, 크로스오브젝트 수식, Check Syntax, 계산 필드, read-only 필드, 10 관계 한도, "다른 필드로 값 자동 계산", "formula 필드 만드는 법", "부모 객체 값 가져오는 수식" | `Admin(어드민)/ObjectsFields(오브젝트·필드)/Formula 필드.md` |
| formula examples, 수식 예제, HYPERLINK IMAGE CASE 예제, Account Rating, Round-Robin, Date 수식, 이미지 링크 수식, 업무별 수식 예제, 109개 고급 수식, 18 업무 카테고리, 커미션 할인 기회 수식, "쓸만한 수식 예제 모음", "이미지 링크 수식 만드는 법", "케이스 커미션 수식 예제" | `Admin(어드민)/ObjectsFields(오브젝트·필드)/Formula 필드 예제 카탈로그.md` |
| formula functions, formula operators, 수식 함수, 수식 연산자, ISPICKVAL CONTAINS BLANKVALUE VLOOKUP REGEX PREDICT, 함수 컨텍스트별, ADDMONTHS TEXT MOD, Math Logical Text 연산자, Date&Time Summary Advanced 함수, 119종 전수, "수식 함수 목록", "이 함수 어디서 쓰나", "수식 연산자 종류" | `Admin(어드민)/ObjectsFields(오브젝트·필드)/Formula 연산자·함수 레퍼런스.md` |
| Roll-Up Summary, 롤업 요약 필드, 집계 필드, COUNT SUM MIN MAX, master-detail 집계, 자식 레코드 합계, 부모 필드 집계, "자식 레코드 개수 세기", "detail 레코드 합계 구하기", "master 레코드에서 집계" | `Admin(어드민)/ObjectsFields(오브젝트·필드)/Roll-Up Summary 필드.md` |
| Page Layouts, 페이지 레이아웃, enhanced page layout editor, mini page layout, feed-based layout, 레이아웃 할당, 필드 배치, 버튼 관련목록 배치, 프로파일 레코드타입 레이아웃, "레코드 페이지에 필드 배치", "페이지 레이아웃 만드는 법", "프로파일별 레이아웃 할당" | `Admin(어드민)/UI(사용자인터페이스)/Page Layouts (페이지 레이아웃).md` |
| Record Types, 레코드 타입, business process, 비즈니스 프로세스, Master record type, sales process, support process, 레코드 타입 할당, 피클리스트 값 제어, "사용자별 다른 페이지 레이아웃", "레코드 타입 만드는 법", "비즈니스 프로세스 분기" | `Admin(어드민)/ObjectsFields(오브젝트·필드)/Record Types (레코드 타입).md` |
| Duplicate Rules, Matching Rules, 중복 규칙, 매칭 규칙, duplicate management, match key, matching method, Allow Block Report, 중복 방지, 데이터 품질, "중복 레코드 막는 법", "매칭 규칙으로 중복 탐지", "중복 발견 시 차단" | `Admin(어드민)/Data(데이터관리)/Duplicate & Matching Rules (중복·매칭 규칙).md` |
| Schema Builder, 스키마 빌더, data model, 데이터 모델, ERD, custom object 관계, 스키마 시각화, 드래그앤드롭 오브젝트, "데이터 모델 시각화", "오브젝트 관계 보기", "스키마 빌더로 필드 추가" | `Admin(어드민)/ObjectsFields(오브젝트·필드)/Schema Builder (스키마 빌더).md` |
...
| Change Data Capture, CDC, 변경 데이터 캡처, change event 구독, 구독 채널명, /data/ChangeEvents, ChangeEvents 채널, ChangeEvent, CDC 엔티티 선택, CDC란 무엇, transaction-based replication, 이벤트 버스 보존, 외부 시스템 데이터 동기화, 레코드 변경 이벤트 수신, 어떤 레코드가 바뀌었는지 외부에 알리기 | `Integration(통합)/Change Data Capture — 개요·채널 구독.md` |
| CDC 커스텀 채널, Change Data Capture custom channel, __chn, PlatformEventChannel, PlatformEventChannelMember, 채널 멤버, 여러 객체 한 채널로 묶기, 커스텀 채널 구독, compose streams, CDC ERD, SalesEvents__chn, 채널에 엔티티 추가 | `Integration(통합)/Change Data Capture — 커스텀 채널.md` |
| ChangeEventHeader, changeType enum, CREATE UPDATE DELETE UNDELETE, GAP_CREATE, GAP_OVERFLOW, gap event, overflow event, merged change event, 병합 이벤트, 누락 이벤트, compound field, commitNumber commitTimestamp transactionKey, changedFields, CDC 이벤트 메시지 구조, CDC 이벤트 놓침, CDC 메시지 필드 | `Integration(통합)/Change Data Capture — 이벤트 메시지·Gap·Overflow.md` |
| CDC enrichment, enriched fields, 강화 필드, always-included fields, 항상 포함 필드, CDC 채널 필터, filter expression, filterExpression, 필터 표현식, filter operators, field types 필터, EnrichedFields, FIELD_INTEGRITY_EXCEPTION, 필터 한도, 이벤트 필드 보강 | `Integration(통합)/Change Data Capture — Enrichment·필터링.md` |
| CDC 고려사항, CDC allocations, CDC 할당량, event delivery limit, 이벤트 전달 한도, PlatformEventUsageMetric, CDC 보안 권한, View All Data FLS, CDC monitoring, CDC 사용량 모니터링, 표준 객체 CDC, standard object special behaviors, Person Account CDC, Lead Conversion CDC, Task Event CDC, encrypted change events, CDC 지원 객체 | `Integration(통합)/Change Data Capture — 고려사항·할당량·표준객체.md` |
| Outbound Messaging, 아웃바운드 메시지, notifications() SOAP callback, outbound message WSDL, Ack listener, `<Ack>true</Ack>`, 워크플로 SOAP 콜백, Send Session ID, 세션 ID 전송, at-least-once 재시도, 100 msg per notification, 24시간 큐 2시간 재시도, 외부 시스템에 SOAP 푸시, 워크플로에서 외부 알림, 승인 프로세스 outbound message | `Integration(통합)/Outbound Messaging (아웃바운드 메시지) — SOAP 콜백·WSDL·리스너.md` |
| Salesforce Connect, Cross-Org adapter, 크로스오르그 어댑터, provider subscriber org, writable external objects, 쓰기 가능 외부 객체, External Change Data Capture, 외부 변경 데이터 캡처, OData 4.0 adapter, OData 2.0 4.01, GraphQL adapter, SQL adapter, DynamoDB adapter, Custom Apex adapter, Salesforce Connect 한도, 100k rows/hr, 200 external objects, 4 joins, 4000 char token, 2000 rows per page, external object license connection, 다른 org 데이터 접근, external object 쓰기, 데이터 가상화 심화 | `Integration(통합)/Salesforce Connect — 어댑터·Cross-Org·writable·External CDC.md` |
