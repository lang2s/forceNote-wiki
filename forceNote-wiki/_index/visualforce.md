...
| VF AJAX LWC 마이그레이션, actionPoller LWC 대체, actionFunction LWC, actionSupport LWC, reRender LWC 대체, actionStatus LWC, LWC 폴링, LWC polling, setInterval LWC, clearInterval disconnectedCallback, 주기적 서버 폴링, empApi Platform Events 푸시 전환, VF 액션 컴포넌트 LWC 대응 | `Visualforce(비주얼포스)/VF AJAX 패턴 → LWC 대응.md` |
| VF to LWC, Visualforce LWC 마이그레이션, VF 페이지 이관, VF 현대화, keep VF, VF 유지 결정, keep-VF 결정 매트릭스, @RemoteAction @AuraEnabled 이관, RemoteObjects uiRecordApi wire, standardController LDS, VF를 LWC로 어떻게 옮기나, 무엇을 VF로 남기나, PDF 렌더 이메일 템플릿 LWC 대체, 단계적 이관 전략, leaf-first 전환 | `Visualforce(비주얼포스)/VF → LWC 마이그레이션 전략.md` |
