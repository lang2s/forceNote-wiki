...
- [[Agent Script 패턴 — 변수·리스트·리소스 참조·시스템 오버라이드]] — 변수·리스트·리소스 참조·시스템 오버라이드·멀티턴 워크플로우 실전 패턴
- [[Agent Script 메타데이터 배포 (DX·패키징)]] — 작성한 에이전트를 다른 org로 retrieve/deploy하는 메타데이터 타입·sf CLI 워크플로우
