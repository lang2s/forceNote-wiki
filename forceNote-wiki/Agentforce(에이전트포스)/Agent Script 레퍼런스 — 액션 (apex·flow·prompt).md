...
## 표시·플래너 제어 속성 (is_displayable · is_used_by_planner · is_user_input · progress_indicator_message)

위 표의 `filter_from_agent`(agent context 포함 여부)·`require_user_confirmation`·`include_in_progress_indicator` 외에도, 액션이 **CLT/Prompt 환경에서 무엇을 사용자에게 표시하고, 플래너가 생성물을 어떻게 소비하는지**를 좌우하는 속성들이 있다. 아래는 recipes 실전 예시에서 실제로 쓰인 속성들이다.

| Property | 위치 | 값 | 역할 |
|---|---|---|---|
| `is_displayable` | 출력 파라미터 | Boolean | `True`면 그 출력 값을 사용자에게 표시(렌더). `False`면 내부용으로만 유지하고 화면에 노출하지 않는다. |
| `is_used_by_planner` | 출력 파라미터 | Boolean | `True`면 플래너(reasoning engine)가 액션 생성물을 **직접 사용**한다 — 중간 변수(`set @variables...`)에 저장하지 않아도 플래너가 그 출력을 바로 참조·활용. |
| `is_user_input` | 입력 파라미터 | Boolean | `True`면 그 입력을 **에디터/CLT 폼으로 사용자에게서 수집**한다(예: 커스텀 Lightning Type 폼으로 구조화 입력 받기). |
| `progress_indicator_message` | 액션 정의 | String | `include_in_progress_indicator: True`일 때 progress indicator에 표시할 커스텀 메시지. 지정하지 않으면 기본 표시. |

> `is_user_input`·`is_displayable`을 실제 **커스텀 LWC UI로 렌더**하는 방법(에디터/디스플레이 컴포넌트 매핑, `complex_data_type_name`과의 연동)은 [[Agentforce 커스텀 Lightning Type — 에이전트 액션 입출력 UI]]가 소관이다 — 여기서는 액션 정의 속성의 스키마·의미만 다룬다.

### 실전 1 — Prompt Template 액션 출력 (is_used_by_planner + is_displayable)

Prompt Template 액션의 출력은 항상 `promptResponse` string이다. 이 recipe는 두 속성을 함께 켜서, 생성된 스케줄을 **플래너가 직접 소비**하면서 동시에 **사용자에게도 표시**한다.
...
여기서 `is_used_by_planner: True`이므로 `promptResponse`를 별도 `@variables`에 저장하지 않고도 플래너가 즉시 답변에 반영한다. `is_displayable: True`라 생성물이 사용자에게 그대로 렌더된다.

> `generatePromptResponse://` 타깃과 `"Input:<Field API Name>"` 바인딩 패턴 자체는 [[Agentforce Prompt Template 액션 — genAiPromptTemplate·Apex 그라운딩]] 참조.

### 실전 2 — CLT 폼 입력 + 선택적 출력 표시 (is_user_input + is_displayable 분기 + progress_indicator_message)

이 recipe는 케이스를 CLT 폼으로 접수한다. 입력은 `is_user_input: True`로 폼 수집, 출력은 **표시할 것/숨길 것을 파라미터별로 분기**하며, 실행 중 커스텀 progress 메시지를 표시한다.
...
- 입력 `case_data`는 `is_user_input: True` + `complex_data_type_name: "c__caseInput"` → 사용자가 CLT 입력 폼(`c__caseInput`)으로 구조화 데이터를 채운다.
- 출력 `case_number`는 `is_displayable: False` → 화면에 직접 표시하지 않고 내부 참조용(케이스 번호는 instructions에서 `@variables.case_number`로 활용).
- 출력 `case_result`는 `is_displayable: True` + `complex_data_type_name: "c__caseResult"` → 생성된 케이스 상세를 CLT 디스플레이 컴포넌트(`c__caseResult`)로 사용자에게 렌더.
- 액션 레벨 `include_in_progress_indicator: True` + `progress_indicator_message: "Submitting your case..."` → 실행 중 그 문구를 progress indicator로 표시.

> 위 예제의 `source: "Submit_Case"`는 이 액션이 매핑되는 기존 Agentforce 액션의 developer name을 가리키는 속성이다.
...
- [[@InvocableMethod 패턴]] — `apex://` 액션이 호출하는 Apex를 `@InvocableMethod`로 노출하는 서버측 짝
- [[Apex MOC]]
- [[Flow MOC]]
- [[스킬 ↔ 위키 토픽 맵]]
