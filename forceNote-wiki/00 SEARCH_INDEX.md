...
| Admin 설정(조직·사용자·보안·데이터·UI·자동화) — 회사 정보·회계연도·영업시간/휴일·멀티통화·설정 감사 추적·사용자 관리·역할 계층·공개 그룹·위임 관리 등 어드민 태스크 | `_index/admin.md` | Admin(어드민)/ 어드민 종합 지식 이니셔티브. **커스터마이제이션 세부(Formula·Roll-Up·Page Layout·Record Type·Data Loader·DIW·Approval)는 `platform` 샤드, OWD/공유는 `security`, Reports/Dashboards는 `analytics` 샤드도 참조** |
