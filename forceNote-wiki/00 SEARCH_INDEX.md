...
| Admin 설정(조직·사용자·보안·데이터·UI·자동화) — 회사 정보·회계연도·영업시간/휴일·멀티통화·설정 감사 추적·사용자 관리·역할 계층·공개 그룹·위임 관리 등 어드민 태스크 | `_index/admin.md` | Admin(어드민)/ 어드민 종합 지식 이니셔티브. **진입점: [[Salesforce 어드민 종합 개요]]** (8도메인 진입 지도). **커스터마이제이션 세부(Formula·Roll-Up·Page Layout·Record Type·Data Loader·DIW·Approval)는 `platform` 샤드, OWD/공유는 `security`, Reports/Dashboards는 `analytics` 샤드도 참조** |
...
| Connect REST API (Chatter/협업 REST — 피드·그룹·사용자·댓글 REST 엔드포인트, ConnectApi의 HTTP 짝) | `_index/connect-rest.md` | `Integration(통합)/ConnectREST(커넥트REST)/` 폴더 — 협업 REST API 클러스터 (개요·요청/응답 규약 + 향후 Feeds·Comments·Groups·Users·Topics). Apex `ConnectApi`(코드)는 `_index/apex-namespaces.md` |
...
| 릴리즈 노트 — Winter '26 이후 (v65~v68 · 2025.10~) | `_index/release.md` | 릴리즈별 변경 (최신 4개 릴리즈) |
| 릴리즈 노트 아카이브 — Winter '24 ~ Summer '25 (v59~v64 · 2023.10~2025.06) | `_index/release-2024-2025.md` | 과거 릴리즈별 변경 (release.md에서 연대순 분할) |
