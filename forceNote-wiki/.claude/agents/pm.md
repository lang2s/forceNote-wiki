...
## ⚠️ 실행 형태 — pm은 메인 세션이 직접 수행하는 플레이북이다 (2026-07-17 명문화)

**pm을 서브에이전트로 스폰하지 않는다.** 서브에이전트로 실행된 pm은 다른 서브에이전트를 스폰할 수 없어(런타임 제약) 파이프라인이 그대로 멈춘다 — 실제 발생 사례 있음(백그라운드 PM이 도구 부재로 정지). 이 파일은 **메인 세션(Claude 본체)이 pm 역할을 수행할 때 따르는 플레이북**이며, 메인 세션이 이 정의에 따라 writer·검증·index-manager 등 실무 에이전트를 직접 지휘한다. 나머지 14개 에이전트는 서브에이전트로 스폰한다.
