...
> **슬림화 (STRUCT-5) — Why:** frontmatter 4필드·코드블록 존재·깨진 wikilink(기계적 존재)·한 줄 요약 등 **결정적으로 판정 가능한 구조 항목은 L1 훅(`scripts/lint-md-file.sh`)이 매 Write/Edit마다 이미 검사**한다(위반 시 exit 2 반려). 여기 도달한 노트는 그 검사를 통과한 전제이므로 qa가 재수행하지 않는다. qa 고유 가치는 **훅이 못 보는 것**(인덱스 등재·파이프라인 산출물 종합·최종 verdict·재작업 라우팅)에 있다.

### 파일 품질 — L1 훅 통과 전제 (재검 불필요, spot-check만)

```
전제(훅이 결정적으로 커버 — 재수행 안 함): frontmatter 4필드 · > 한 줄 요약 · 코드 블록 최소 1개 · ## 관련 노트 섹션 · 깨진 wikilink(기계적 존재)
□ (훅 밖) Tier 3 파일에 [!warning] 경고 블록 있음 — 소스 tier는 훅이 판정 못 하므로 qa가 확인
```

### 인덱스 업데이트

```
□ 주 도메인 `_index/{샤드}.md`에 새 파일의 키워드 행 있음 (라우터가 아니라 샤드)
□ 섹션 MOC에 새 파일 링크 있음
...
| 상황 | 판정 |
|---|---|
| 인덱스 등재·종합 verdict 체크리스트 통과 | ✅ 통과 |
| 경미한 스타일 문제만 있음 | ✅ 통과 (개선 제안 포함) |
| 인덱스 누락(샤드/MOC/index.md 미등재), Tier 3 경고 누락 | ❌ 반려 |
| Source Verifier ❌ 미처리 (API명/시그니처/의미 오연결 등) | ❌ 반려 |
