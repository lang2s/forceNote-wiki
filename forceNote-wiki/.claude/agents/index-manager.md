...
4. 00 Home.md              — **새 폴더/새 도메인이 생겼을 때만**: Layer 0 진입점에 카테고리 섹션/링크 추가.
                             (누락 시 인덱스로는 도달돼도 Home에서 안 보이는 드리프트 발생 — 2026-07 OmniStudio 사례)
```

> 새 폴더·새 샤드가 생겼으면 PM에게 보고에 명시한다 — vault CLAUDE.md의 "Layer 2 샤드 열거"·"Layer 3 표" 갱신은 PM(메인 세션)이 수행한다 (CLAUDE.md는 탐색 파일이 아님).
...
3. 행 추가 후 샤드가 **~300줄 또는 ~40k chars(≈12k 토큰) 중 먼저 도달하는 쪽**을 넘으면 (`wc -l -c`로 확인 — 한국어 행은 줄수보다 토큰이 먼저 터진다):
