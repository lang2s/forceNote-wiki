...
### Pattern E — 위임 경계 누수 (delegation boundary leak)

blueprint가 어떤 주제를 다른 노트 `[[X]]`에 위임(소관 분리)했다면, 이 노트에서는 그 주제의 **본문 깊이를 다시 쓰지 않는다.** 위임 주제는 1줄 포인터(`> ...는 [[X]] 참조`)로만 남기고 메커니즘 재서술을 금지한다.

- **재서술 금지:** 위임된 메커니즘을 "맥락상 필요"라며 본문에서 다시 풀어 설명하면 두 노트가 같은 내용을 중복 보유한다(소관 충돌). source-coverage-checker가 사전 경고하면 그 경고를 본문에 반영한다 — 경고를 무시하고 써서 completeness-validator가 축약하게 만들지 않는다.
- **키워드 거짓 광고 금지:** 제안하는 인덱스/aliases 키워드는 **이 노트 본문에 실제 내용이 있을 때만** 단다. 위임 포인터만 있는 주제(예: "PK chunking은 범위 밖, [[Bulk API 2.0]] 참조")의 키워드는 그 내용을 보유한 노트 쪽에 가야 한다. 위임 링크만 있는 주제를 이 노트 키워드로 광고하면 index↔content 거짓 약속이 된다.
- 자가 점검: 제안한 각 키워드/alias마다 "이 단어로 검색해 온 사용자가 *이 노트 본문*에서 답을 얻는가?"를 확인. 아니면 그 키워드는 위임 대상 노트 소관.
...
□ 폴더명이 English(한글) 형식임
□ created: 에는 **작업 당일 실제 날짜**를 넣는다 (date +%F 로 확인 — 이 파일의 예시 날짜를 복사하지 않는다)
```
