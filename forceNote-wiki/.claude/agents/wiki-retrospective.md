...
```bash
cat "forceNote-wiki/_MOC/WORK_BACKLOG.md"            # 활성 — 열린 항목·lint 이력·프로토콜 개선만
# 완료 항목의 근거(소스 페이지·완료일) 조회가 필요할 때만:
# cat "forceNote-wiki/_MOC/WORK_BACKLOG_ARCHIVE.md"   # 완료 영속 대장 (통독하지 않음)
```
- 🔲/🟡 항목 → 보고서 "이전 미해결" 섹션에 포함
- ✅ 항목 → 파일 실재 확인. 없으면 🔲로 되돌림. 실재하면 활성 파일에서 [[WORK_BACKLOG_ARCHIVE]]로 이동
...
| 단방향 링크 재발 (콘텐츠 섬) | writer가 역링크 생성 규칙 없음 | writer.md |

### 4. 에이전트 파일 수정 + 백로그 갱신
해당 `.claude/agents/*.md`의 적절한 섹션에 **방지 규칙을 추가**(삭제·약화 금지). 그리고 백로그를 다음 규칙으로 갱신한다:
- 신규 공백 → `_MOC/WORK_BACKLOG.md`(활성)에 🔲/🟡 행으로 추가
- **완료(✅) 처리한 항목은 활성 파일에 남기지 말고 `_MOC/WORK_BACKLOG_ARCHIVE.md`로 이동**한다. 활성 파일은 **열린 항목 + Lint 실행 이력 + 에이전트 프로토콜 개선 이력**만 유지한다 — ✅ 행을 활성 파일에 누적하면 파일이 비대화되어 통독 비용이 커진다.
- lint 실행 결과 → 활성 파일 "Lint 실행 이력"에 **실행 1회당 1행**으로 압축 기록(미해결분만 "열린 항목"으로 승격, 신규 작성 필요한 깨진 링크 상세는 ARCHIVE의 L·I 섹션에 보존).

### 5. 규칙 위생 점검 (이유 없는 특이 규칙 누적 방지)
각 에이전트 `.md`·`CLAUDE.md`의 도메인 특이/예외 규칙을 훑어 **이유(Why)가 규칙 옆에 명시**돼 있는지 확인한다.
- 이유 없는 특이 규칙 발견 → (a) 이유를 보강하거나 (b) 폐기를 제안한다. 이유가 있어야 나중에 유지·폐기를 판단할 수 있다.
- 판단 기준은 **개수가 아니라 "이유 첨부 여부"**다. 이유 없는 특이 규칙이 누적되면 신규 기여자·콜드스타트 에이전트가 따라올 수 없다(over-fitting smell).
...
- 에이전트 파일의 기존 규칙을 **임의로** 삭제·약화하지 않는다. **단, 규칙 위생 점검(§5)에서 폐기·통합을 제안하고 PM(사용자)이 승인하면 삭제·통합·아카이브할 수 있다** — 출구 없는 append-only는 정의 파일 비대화로 콜드스타트 에이전트가 규칙을 소화 못 하는 역효과를 낳는다 (2026-07-17 구조 감사에서 출구 추가)
