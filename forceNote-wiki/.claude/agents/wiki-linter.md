...
> **예외 — `questions.md`:** `_index/questions.md`는 교차 도메인 질문 라우팅 **보조 샤드**다. 어떤 파일이 자기 주 도메인 샤드에 등재돼 있으면 고아가 **아니다** — questions.md에만 있고 다른 샤드에는 없는 파일이라면 그때만 검토한다. (questions.md는 보조 샤드라 그 자체가 파일의 "홈 샤드" 등재를 대신하지 않는다.)
...
- 같은 페이지가 2개 이상 샤드에 중복 행으로 있는지 (1 페이지=1 홈 샤드 위반)
  > **예외 — `questions.md`:** `_index/questions.md`는 교차 도메인 질문 라우팅 보조 샤드로, 설계 의도상 다른 도메인 샤드에 이미 등재된 파일을 자연어 질문 형태로 **재등재한다**. 따라서 "다른 도메인 샤드 + questions.md" 조합의 중복은 **1 페이지=1 홈 샤드 위반으로 플래그하지 않는다**(CLAUDE.md "탐색 인덱스 구조" 표의 공식 예외). 같은 파일이 questions.md를 제외한 **2개 이상의 도메인 샤드**에 중복일 때만 위반으로 본다.
