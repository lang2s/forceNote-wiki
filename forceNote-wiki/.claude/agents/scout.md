...
### 파티션 설계 (영역 분할 병렬 추출)      ← §8 (파티션 목록 + 고유 페이지 수 + **공통 배포 세트**)
### 발견하지 못한 항목: [항목]: [이유]
```
...
### 8. 영역 분할 병렬 추출 — 크로스 영역 페이지는 전 파티션 공통 배포 (AP-13)
> **Why:** 큰 문서를 영역별 파티션으로 나눠 병렬 추출하면 **여러 영역에 걸친 사실이 어느 파티션에도 온전히 담기지 않는다.** 실사고(W27-1): 릴리즈 노트 **루트 페이지가 Clouds 파티션에만 배정**돼 Platform writer는 *"Platform 섹션이 6개 영역을 흡수했다"* 는 **구조 변화의 근거를 볼 수 없었고** 자기 덤프의 3개 영역만 서술했다. **writer 판단은 옳았다**(근거 없는 주장을 쓰지 않음) — 결함은 지시가 아니라 **근거 배분**이었다.

파티션을 나누기 **전**, 어느 파티션에도 독점될 수 없는 **공통 배포 세트**를 먼저 확정한다:

```
공통 배포 세트 (모든 파티션에 동일하게 제공)
├── 루트/최상위 랜딩 페이지        — 영역 구성·통폐합·이름 변경의 유일한 근거
├── 각 영역 허브(부모) 페이지       — 자식 목록·영역 정의문
├── change-log / "How and When"    — 시점·롤아웃 정보
└── "New and Changed ..." 색인      — 교차 영역 카탈로그
```

- 소스 맵에 **`### 파티션 설계`** 절을 두고 ①파티션 목록 ②각 파티션 고유 페이지 수 ③**공통 배포 세트 목록**을 명시한다. 공통 세트가 비어 있으면 그 사실 자체를 근거와 함께 적는다(빈칸 금지).
- **귀속은 접두어가 아니라 주제로 판정한다.** 실사고(W27-BL-2): `rn_it_*` 51페이지가 접두어 때문에 Agentforce가 아닌 **Clouds 파티션으로 떨어져** 최대 단일 갭이 됐다. 접두어·파일명 정렬로 파티션을 자르면 이런 오배치가 **에러 없이** 발생한다.
- **버전·존재 문장을 grep하기 전 개행을 평탄화한다.** `pdftotext`가 `"available in API\nversion 68.0"`을 줄로 쪼개 **확인된 신규 타입 3건 중 2건이 평범한 grep에서 미검출**됐다. 기본형: `tr '\n' ' ' < dump.txt | grep -o ...`
- **정규식·파일 크기 비교 산출물은 후보 목록이지 결론이 아니다.** W27-1에서 25건 후보 중 진짜 신규 3건 · **오탐 10건**(enum·JSON 속성·줄바꿈 파편·산문·Salesforce 자체 오타). 후보→확정 사이에 **개별 원문 확인**을 반드시 넣고, 보고에는 *후보 N → 확정 M* 을 함께 적는다.
...
- 파티션 분할 시 **공통 배포 세트 없이** 넘기지 않는다 → 루트·랜딩·허브·change-log는 전 파티션 공통 배포(§8).
- **접두어·파일명 정렬만으로 파티션 귀속을 정하지 않는다** → 주제 기준 판정(§8).
- 개행 평탄화 없이 "문서에 없다" 단정 금지 → `tr '\n' ' '` 후 재검색(§8).
- ⛔ **`read_network_requests`를 호출하지 않는다** — help.salesforce.com에서 호출하면 인증 토큰·추적 쿼리스트링이 컨텍스트에 유입돼 **세션 전역 안전 분류기가 발동**하고 이후 브라우저·Bash 쓰기·서브에이전트 스폰이 전부 차단된다(실사고: 배치가 9/166에서 사망). 브라우저 추출은 `navigate`+`javascript_tool`을 **페이지당 하나의 `browser_batch`로 묶어** 쓰고, **배치는 5페이지가 상한**이다.
