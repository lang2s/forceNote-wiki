...
## 멀티사이클 단일-PDF 인제스트 — 기작성 객체 중복 제외 (재발 방지 규칙)

> **Why (AP-10):** 한 대형 PDF를 **여러 사이클로 도메인 그룹 분할**해 인제스트할 때(ING-26이 Tooling API Ch4 알파벳 객체 카탈로그를 C4-1~C4-9 도메인 그룹으로 슬라이스), **한 객체가 여러 도메인에 정당하게 속해** 나중 그룹의 로스터에 다시 잡히는데 — 그 객체가 **이미 앞 사이클 노트에 작성됐을 수 있다.** 로스터를 그대로 신뢰하면 writer가 그 객체 섹션을 다시 써서 **두 노트에 중복 객체 섹션**이 생긴다. 증명: **C4-7(Cycle 10)** 로스터 41객체에 `Certificate`(C4-5 보안 기작성)·`IconDefinition`(C4-4 UI 기작성)·`ProcessFlowMigration`(C4-3 자동화 기작성) **3종 중복** 포함 / **C4-8(Cycle 11)** 로스터 중간에 `PlatformEventMigration`(C4-7a 운영 기작성)이 PlatformEventChannelMember와 PlatformEventSubscriberConfig 사이에 끼어 있었음 — 둘 다 writer 투입 전 **수동 grep으로만** 잡힘. 이 제외는 표준 프로토콜이 아니라 메인 세션의 즉석 조치였으므로, 프로젝트 메모리를 읽지 않는 **새 scout가 다음 멀티사이클 인제스트**(chatter_rest 2350p·extend_click_automate 1027p·lightning 553p·pages_dev 등)를 돌리면 재발한다.

이미 일부가 여러 사이클로 인제스트된 PDF의 한 도메인-슬라이스 로스터를 다룰 때는, **로스터 확정 전** 각 후보 객체가 형제 노트에 이미 작성됐는지 교차 확인한다:

```bash
# 같은 인제스트의 형제 노트들에서 각 후보 객체 섹션이 이미 작성됐는지 확인
grep -l "^### <ObjectName>" "<wiki-folder>/"*.md   # 객체 헤딩 규약에 맞게 패턴 조정
```

- 로스터의 각 객체에 처리 표식을 **명시**한다: **REWRITE**(신규 작성) / **(기작성 → 링크만, 어느 노트에 있는지 명시)** / **(DELEGATE)**.
- 소스 맵 출력에 **'기작성으로 제외된 후보' 목록**을 별도로 보고한다(어느 형제 노트에 있는지 함께).
- **계획/추정 객체 수는 신호일 뿐**이다. 실제 개수는 PDF 로스터에서 직접 도출하고, 추정치와의 불일치를 보고한다 — 추정치에 억지로 맞추지 않는다(ING-26 전 사이클에서 추정이 양방향으로 빗나감: C4-3 27→19·C4-4 32→22·C4-2 5→5·C4-7 32→41·C4-8 18→7). 기작성 객체를 제외한 **뒤**에 최종 신규 개수를 센다.

소스 맵 출력 예:

```
### 멀티사이클 중복 제외 (단일-PDF 도메인 슬라이스)
- 인제스트: ING-26 Tooling API Ch4 / 이번 그룹: C4-9a Service
- 로스터 원본: N개 (PDF 로스터 실측)
- 기작성으로 제외: `Certificate`(→ C4-5 보안 노트), `IconDefinition`(→ C4-4 UI 노트) ...
- REWRITE 최종: M개 (= N − 제외)
- 계획 추정치: K개 → 실측 M개 (불일치 ±X, 추정 강제 안 함)
```
...
- 멀티사이클 단일-PDF 인제스트에서 도메인-슬라이스 로스터를 형제 노트 grep 없이 그대로 넘기지 않는다 — 기작성 객체를 제외한 뒤 REWRITE 최종 개수를 보고한다(계획 추정치에 억지로 맞추지 않는다)
