...
> **lint 스코프 제외 (공식):** `문서/` 트리(및 하위 `문서/Project 분석`·`문서/Team Member`·`문서/Interface`·`문서/Developer` 등)는 **Salesforce 백과사전 위키 콘텐츠가 아니라 비-Salesforce 사내 프로젝트 분석 문서**다. 위키 nav(라우터·샤드·MOC·index)와 분리돼 있고 어느 샤드에도 등재되지 않는다. 따라서 lint의 폴더명 규칙(English(한글))·고아 파일·단방향 링크·MOC 누락 검사에서 **제외**한다(순한글 폴더명·미등재는 의도된 상태이므로 위반 아님). 위키 콘텐츠 트리는 `forceNote-wiki/` 하위 도메인 폴더뿐이며 `문서/`는 그 스코프 밖이다.
