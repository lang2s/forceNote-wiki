...
## ⛔ 절대 규칙 — 사용자용 참고 문서 생성 (위반 금지)

**발동 조건:** 위키 내용에 대한 질문에 답하는 과정에서, 사용자가 결과물을 **위키에 등록하지 말고 따로/참고용으로 "문서로 만들어줘"** 라고 요청할 때.
> 단, 사용자가 **"위키에 추가해줘 / 위키 문서로 만들어줘"** 처럼 위키 등록을 명시하면 이 규칙은 **적용되지 않는다** — 그건 정상 위키 작업이므로 팀 파이프라인으로 처리한다.

발동 조건에 해당하면:

1. **위키에 파일을 추가하지 않는다 — 절대로.** 그 문서는 위키 콘텐츠가 아니라 **사용자 확인용 참고자료(reference)** 다. 위키 콘텐츠 트리에 `.md`로 저장하거나 색인(샤드·MOC·index)에 등록하는 행위 일체 금지.
2. **기본 포맷은 HTML(`.html`).** 단, 사용자가 다른 포맷(PDF·Markdown 등)을 **명시**하면 그 포맷을 따른다.
3. **저장 위치는 위키 콘텐츠 트리(`forceNote-wiki/`) 바깥의 `_user-docs/` 폴더.** 이 폴더는 git 루트(= `forceNote-wiki/`의 **상위** 폴더)에 있으며 `.gitignore`에 등록돼 커밋되지 않는다. 만든 뒤 사용자에게 파일 경로를 알린다. (사용자가 다른 경로를 지정하면 그곳에 저장.)
   > ⚠️ 구조 주의: **git 루트는 `forceNote-wiki/`가 아니라 그 상위 폴더**다. git 루트엔 위키(`forceNote-wiki/`)·원본 PDF(`Salesforce Documents/`)·`_user-docs/`가 함께 있고, 이 중 **위키 콘텐츠 트리는 `forceNote-wiki/` 하나뿐**이다. 따라서 `forceNote-wiki/` 밖에만 두면 위키는 오염되지 않는다.

> **우선순위:** 이 규칙은 "탐색·분석 결과 보존"(가치 있는 분석을 위키에 .md로 저장)보다 **우선**한다. 사용자가 "따로/참고용 문서로 만들어줘"라고 하면 위키 저장이 아니라 외부 참고자료 생성이다. 반대로 "위키에 보존/추가"가 의도면 보존 규칙을 따른다.
...
Layer 2  _index/{도메인}.md      — 키워드 → 파일 경로 샤드 (frontend/frontend-basecomponents/visualforce/apex-core/apex-namespaces/platform/platform-devops/platform-devops-2gp/platform-devops-tooling/release/sobject-reference/service/scheduler/field-service/cpq/analytics/security/agent-skills/questions)
...
코드 예제·다이어그램·ASCII art·예시 JSON 등 **직접 만든 모든 구조물**은 작성 전 원본과 대조하거나 마커를 단다.

| 자료 유형 | 검증 방법 |
|---|---|
| 로컬 소스 코드 발췌 | 해당 파일을 Read 도구로 열어 메서드명·파라미터·반환 타입이 실제와 일치하는지 확인 |
| 공식 문서(PDF) 발췌 | PDF에서 인용한 코드 블록과 작성 내용 대조 |
| 직접 작성한 구조 예시 (코드) | 코드 블록 첫 줄에 `// 구조 예시 — 실제 동작 코드 아님` 주석 |
...
| 키워드 샤드 | `_index/frontend.md` · `_index/frontend-basecomponents.md` · `_index/visualforce.md` · `_index/apex-core.md` · `_index/apex-namespaces.md` · `_index/platform.md` · `_index/platform-devops.md` · `_index/platform-devops-2gp.md` · `_index/platform-devops-tooling.md` · `_index/release.md` · `_index/sobject-reference.md` · `_index/service.md` · `_index/scheduler.md` · `_index/field-service.md` · `_index/cpq.md` · `_index/analytics.md` · `_index/security.md` · `_index/agent-skills.md` · `_index/questions.md` |
...
### sf-skill 증강 답변 (지식 = 위키 · 실행 = 스킬)

Salesforce **실행형/생성형 질문**(예: "테스트 클래스 만들어줘", "배포해줘", "SLDS 적용", "Flow 생성", 라이브 org 작업)에는 위키 지식에 더해 대응 **sf-skill**(`AgentSkills(에이전트스킬)/sf-skills/`)을 활용한다. 단 아래 규율을 지킨다.

| # | 규칙 |
|---|---|
| 1 | **위키 우선(지식).** "무엇/왜/패턴" 지식 질문은 위키로 검증된 답을 먼저 준다. sf-skill의 *지식*은 이미 위키가 흡수했으므로 지식 질문에 스킬을 끌어올 필요 없음. |
| 2 | **스킬은 실행형일 때(역량).** "만들어줘/생성/배포/쿼리/스캐폴딩/고쳐줘" 또는 라이브 org 작업이면 대응 스킬을 활용/안내. |
| 3 | **현재 "안내" 모드.** sf-skills는 아직 실행 스킬로 설치되지 않음(`.claude/skills` 없음) → 위키의 해당 sf-skill 노트와 [[스킬 ↔ 위키 토픽 맵]]으로 *적합 스킬을 가리키고* 절차를 안내한다. 설치 후엔 "실행" 모드로 전환. |
| 4 | **출처 라벨 분리(필수).** 한 답에 섞되 구분한다: `📘 위키(Tier N): …`(검증된 사실) vs `⚙️ sf-skill: …`(절차·생성물, 위키 검증 전). 스킬 산출물은 위키 패턴 노트로 교차검증 권장. |
| 5 | **디스패처.** [[스킬 ↔ 위키 토픽 맵]](`AgentSkills(에이전트스킬)/sf-skills/`)이 "질문 주제 → 대응 스킬 + 위키 패턴" 라우팅 표. 강 겹침 노트는 스킬↔위키 양방향 링크돼 있어 어느 쪽에서 출발해도 반대쪽이 잡힌다. |

> 카운터파트가 없는 스킬 도메인(Agentforce·Data 360·OmniStudio·React UI Bundle·Code Analyzer 등 35개)은 위키 미커버 = 콘텐츠 갭. 맵 노트의 "콘텐츠 갭" 섹션 참조.
...
2. 단방향 링크 탐지     — A→[[B]] 인데 B→[[A]] 가 없는 쌍. **단 spoke→hub(개요·MOC·index·카탈로그·고fan-in 허브로의 링크)는 의도된 단방향이라 플래그 안 함** — 대상이 허브·index로 도달 가능하면 콘텐츠 섬 아님. 형제(peer) 노트끼리 상호 관련인데 한쪽만 링크일 때만 양방향 보완 (writer 작성 시점 역링크 규칙과 동일 예외)
...
- ⚠️ 단, 사용자가 **"위키에 추가하지 말고 따로/참고용 문서로 만들어줘"** 라고 하면 이 보존 규칙이 아니라 최상단 **"⛔ 절대 규칙 — 사용자용 참고 문서 생성"**(외부 HTML 참고자료)을 따른다.
