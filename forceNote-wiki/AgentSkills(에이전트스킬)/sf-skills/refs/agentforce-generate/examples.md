---
tags: [agent-skill, sf-skills, reference, agentforce, examples]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/examples.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Complete Agent Examples, 완전한 에이전트 예제, service employee multi-subagent]
---

# Complete Agent Examples — 완전한 에이전트 예제

> 배포 가능한 완전한 Agent Script 예제 모음: 최소 서비스 에이전트, 최소 직원(Employee) 에이전트, 액션을 포함한 멀티 서브에이전트 에이전트. 서비스 vs 직원 에이전트의 구조 차이를 명시한다.
