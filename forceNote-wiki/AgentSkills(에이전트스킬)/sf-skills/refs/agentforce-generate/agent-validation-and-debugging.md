---
tags: [agent-skill, sf-skills, reference, agentforce, debugging]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/agent-validation-and-debugging.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Agent Validation and Debugging, 에이전트 검증과 디버깅, session traces, grounding, preview]
---

# Agent Validation and Debugging — 에이전트 검증과 디버깅 레퍼런스

> `sf agent validate`로 Agent Script 검증, 오류 분류·예방, preview(simulated vs live) 워크플로우, session trace 분석(step 타입·jq 진단), 증상→근본원인 진단 패턴, 8단계 진단 워크플로우, grounding 재시도 메커니즘과 수정 접근.
