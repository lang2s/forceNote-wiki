---
tags: [agent-skill, sf-skills, reference, agentforce, diagrams]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/agent-subagent-map-diagrams.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Subagent Map Diagrams, 서브에이전트 맵 다이어그램, mermaid graph TD, 에이전트 구조 시각화]
---

# Agent Subagent Map Diagrams — 서브에이전트 맵 다이어그램 레퍼런스

> Agent Spec의 핵심 시각 산출물인 Subagent Map(Mermaid flowchart) 작성 규칙 — graph TD 방향, 노드 타입(router/subagent/action/decision/variable/utility), 패턴, Local_Info_Agent 완전 예제, 검증 체크리스트, 안티패턴.
