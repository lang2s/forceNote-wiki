---
tags: [agent-skill, sf-skills, reference, agentforce, discover]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/discover-reference.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Discover Target Discovery, 타겟 디스커버리, agent target validation]
---

# Discover - Target Discovery Reference — 타겟 디스커버리 레퍼런스

> Agent Script `.agent` 파일의 타겟(`flow://`·`apex://`·`retriever://` 등)이 실제 Salesforce 조직에 존재하는지 검증하고, 누락 타겟에 대해 퍼지 매칭 제안을 제공한다.
