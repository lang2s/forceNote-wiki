---
tags: [agent-skill, sf-skills, reference, agentforce, instruction-resolution]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/instruction-resolution.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Instruction Resolution, 지시 해석, three phases runtime prompt]
---

# Instruction Resolution — Agent Script 지시 해석 (런타임)

> Agent Script 지시가 런타임에서 정적 텍스트로부터 동적 LLM 프롬프트로 처리되는 방식: Phase 1(Pre-LLM) → Phase 2(LLM 추론) → Phase 3(Post-Action Loop)의 3단계, 권장 지시 순서, 패턴·안티패턴, 트레이스 분석까지.
