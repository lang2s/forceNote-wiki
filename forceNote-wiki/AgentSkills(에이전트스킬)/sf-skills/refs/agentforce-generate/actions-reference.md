---
tags: [agent-skill, sf-skills, reference, agentforce, actions]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/actions-reference.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Actions Reference, 에이전트 액션 레퍼런스, Flow Apex API 액션, escalation routing]
---

# Actions Reference — 에이전트 액션 종합 레퍼런스

> Agentforce의 Agent Actions 완전 가이드 — 액션 properties, 모든 target type, 호출 방식(actions 블록 vs run), Flow/Apex/API 액션, escalation routing(connection 블록), 교차 스킬 오케스트레이션, 트러블슈팅.
...
1. **Create Named Credential** (via integration-connectivity-generate skill)
...
├──────────────────────────────────────────────────────────────┤
│  1. integration-connectivity-connected-app-configure  → Connected App (if OAuth needed) │
│  2. integration-connectivity-generate → Named Credential + External Service │
│  3. platform-apex-generate            → @InvocableMethod (if custom logic)  │
│  4. automation-flow-generate            → Flow wrapper (HTTP Callout / Apex)  │
