---
tags: [agent-skill, sf-skills, reference, agentforce, scoring-rubric]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/scoring-rubric.md, 공식 Salesforce)
created: 2026-06-27
aliases: [100-Point Scoring Rubric, 100점 채점 루브릭, agent quality scoring]
---

# 100-Point Scoring Rubric — 생성 에이전트 100점 채점 루브릭

> 생성된 모든 에이전트를 사용자에게 제시하기 전 이 루브릭으로 채점한다. 7개 카테고리(구조·안전·결정적 로직·지시 해석·FSM·액션 구성·배포 준비)에 배점을 두고 점수대별 조치를 안내한다.
...
```
// 구조 예시 — 실제 채점 출력 아님 (루브릭 적용 형태)
Score: 82/100 (Good with minor issues)
- Structure & Syntax: 15/15
- Safety & Responsible AI: 10/15  (1 WARN: no AI disclosure → -5)
- Deterministic Logic: 18/20
- Instruction Resolution: 17/20
- FSM Architecture: 10/10
- Action Configuration: 7/10
- Deployment Readiness: 5/10  (missing default_agent_user)
```
