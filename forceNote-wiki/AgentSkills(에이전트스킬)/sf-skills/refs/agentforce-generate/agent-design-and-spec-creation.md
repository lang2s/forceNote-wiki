---
tags: [agent-skill, sf-skills, reference, agentforce, agent-spec]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/agent-design-and-spec-creation.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Agent Design and Spec Creation, 에이전트 설계와 스펙 작성, subagent architecture, gating patterns]
---

# Agent Design and Spec Creation — 에이전트 설계와 스펙 작성

> Agent Spec(설계 문서) 구조와 라이프사이클, discovery 질문, 환경 사전 요건, subagent 아키텍처, 액션 매핑(Apex/Flow/Prompt 타입 매핑 포함), transition 패턴, deterministic vs subjective 흐름 제어, gating 패턴, action loop 방지.
