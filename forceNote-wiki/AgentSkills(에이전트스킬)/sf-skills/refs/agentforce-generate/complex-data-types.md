---
tags: [agent-skill, sf-skills, reference, agentforce, complex-data-types]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/complex-data-types.md, 공식 Salesforce)
created: 2026-06-27
aliases: [complex_data_type_name mapping, 복합 데이터 타입 매핑, Lightning type mapping]
---

# `complex_data_type_name` Mapping Table — 복합 데이터 타입 매핑표

> 액션 입력/출력 정의 시 "컴파일 에러 1위 원인"인 `complex_data_type_name`을 올바르게 고르기 위한 결정 트리·전체 매핑표·Agent Script ↔ Lightning 타입 매핑.
...
```agentscript
// 구조 예시 — 실제 동작 코드 아님 (위 매핑표를 액션 I/O에 적용한 형태)
outputs:
   order_count: object
      complex_data_type_name: "lightning__integerType"   # apex:// target → integerType
   total_amount: object
      complex_data_type_name: "lightning__doubleType"     # decimal value
```
