---
tags: [agent-skill, sf-skills, reference, agentforce, patterns]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/architecture-patterns.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Architecture Patterns, 에이전트 아키텍처 패턴, hub-and-spoke, verification gate, post-action loop]
---

# Architecture Patterns — 에이전트 아키텍처 패턴

> SKILL.md Section 8에서 추출된 에이전트 아키텍처 패턴 — Hub-and-Spoke(가장 흔함), Verification Gate, Post-Action Loop, Single Subagent의 선택 기준과 구현, hub-and-spoke 마이그레이션, multi-intent 처리, 불완전 입력·action chain 제어.
