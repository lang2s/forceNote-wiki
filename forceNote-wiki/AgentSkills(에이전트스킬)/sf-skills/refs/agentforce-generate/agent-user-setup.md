---
tags: [agent-skill, sf-skills, reference, agentforce, permissions]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/agent-user-setup.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Agent User Setup, 에이전트 유저 셋업, Einstein Agent User, permission set 프로비저닝]
---

# Agent User Setup & Permission Model — 에이전트 유저 셋업 및 권한 모델

> Einstein Agent User와 permission set의 완전한 프로비저닝 워크플로우 — 에이전트 타입 결정 매트릭스, CLI 패스트 트랙, Service Agent 6단계 셋업, Employee Agent 셋업, 자동 생성 PS 경고, 검증 체크리스트, 트러블슈팅, PS XML 템플릿.
