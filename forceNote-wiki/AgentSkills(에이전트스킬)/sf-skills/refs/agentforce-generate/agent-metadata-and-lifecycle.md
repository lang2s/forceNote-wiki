---
tags: [agent-skill, sf-skills, reference, agentforce, lifecycle]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/agent-metadata-and-lifecycle.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Agent Metadata and Lifecycle, 에이전트 메타데이터와 라이프사이클, AiAuthoringBundle, deploy vs publish]
---

# Agent Metadata and Lifecycle — 에이전트 메타데이터와 라이프사이클

> Agent Script 에이전트의 두 메타데이터 도메인(authoring AiAuthoringBundle vs runtime Bot/BotVersion/GenAiPlannerBundle), generate→deploy→publish→activate→test 파이프라인, deploy와 publish의 결정적 차이, 라이프사이클 운영(retrieve/delete/rename/test/open) 전체 레퍼런스.
