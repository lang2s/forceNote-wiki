---
tags: [agent-skill, sf-skills, reference, agentforce, agent-script]
source: forcedotcom/sf-skills (skills/agentforce-generate/references/agent-script-core-language.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Agent Script Core Language, 에이전트 스크립트 핵심 언어, 블록 순서, variables, actions, anti-patterns]
---

# Agent Script: Core Language Reference — 에이전트 스크립트 핵심 언어 레퍼런스

> Agent Script 언어의 실행 모델(deterministic resolution + LLM reasoning), 파일 구조와 블록 순서, 네이밍/포맷 규칙, 표현식·연산자, system/config 블록, variables, subagents, reasoning instructions, flow control, actions, utility 함수, 그리고 안티패턴 모음.
