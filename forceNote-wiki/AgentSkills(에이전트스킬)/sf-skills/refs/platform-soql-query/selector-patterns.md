---
tags: [agent-skill, sf-skills, reference, platform, soql, selector]
source: forcedotcom/sf-skills (skills/platform-soql-query/references/selector-patterns.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Selector Patterns, 셀렉터 패턴, 쿼리 추상화, Query Builder, Mockable Selector, Bulkified Query]
---
# Selector Patterns: Query Abstraction in Vanilla Apex — 셀렉터 패턴(순수 Apex 쿼리 추상화)

> 외부 라이브러리 없이 순수 Apex로 쿼리를 추상화하는 5가지 패턴(기본 셀렉터·공유 모드·모킹·쿼리 빌더·벌크화)으로 테스트성·유지보수성·보안 준수를 높인다.
