---
tags: [agent-skill, sf-skills, reference, platform, soql, field-coverage]
source: forcedotcom/sf-skills (skills/platform-soql-query/references/field-coverage-rules.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Field Coverage Rules, 필드 커버리지, SObjectException, 쿼리하지 않은 필드, 관계 필드 접근]
---
# SOQL Field Coverage Rules — SOQL 필드 커버리지 규칙

> Apex 코드에서 접근하는 모든 필드가 실제로 쿼리됐는지 검증하는 규칙 — LLM 생성 코드에서 흔한 런타임 오류(쿼리되지 않은 필드 접근) 예방.
