...
source: forcedotcom/sf-skills (skills/external-diagram-mermaid-generate/assets/oauth/authorization-code-pkce.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Authorization Code PKCE Flow Template, 인증 코드 PKCE 플로우 템플릿, OAuth PKCE]
---

# Authorization Code + PKCE Flow Template — 인증 코드 + PKCE 플로우 템플릿
> 클라이언트 시크릿을 안전하게 저장할 수 없는 퍼블릭 클라이언트(SPA·모바일·데스크톱)를 위한 OAuth 2.0 인증 코드 + PKCE 플로우 시퀀스 다이어그램 템플릿.
