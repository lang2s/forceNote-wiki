...
source: forcedotcom/sf-skills (skills/external-diagram-mermaid-generate/assets/oauth/client-credentials.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Client Credentials Flow Template, 클라이언트 자격증명 플로우 템플릿, OAuth Client Credentials]
---

# Client Credentials Flow Template — 클라이언트 자격증명 플로우 템플릿
> client_id와 client_secret을 사용하는 서버 간(server-to-server) 인증을 위한 OAuth 2.0 클라이언트 자격증명 그랜트 시퀀스 다이어그램 템플릿.
