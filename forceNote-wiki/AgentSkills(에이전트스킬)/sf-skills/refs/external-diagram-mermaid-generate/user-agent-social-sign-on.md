---
tags: [agent-skill, sf-skills, reference, diagram, mermaid, oauth]
source: forcedotcom/sf-skills (skills/external-diagram-mermaid-generate/assets/oauth/user-agent-social-sign-on.md, 공식 Salesforce)
created: 2026-06-27
aliases: [User-Agent Social Sign-On Flow Template, 유저에이전트 소셜 로그인 플로우 템플릿, OIDC Social Sign-On]
---

# User-Agent with Social Sign-On Flow Template — 유저에이전트 + 소셜 로그인 플로우 템플릿
> Salesforce가 인증 제공자(Authorization Provider)이자 외부 OIDC 제공자(Google·Facebook 등)에 대한 신뢰 당사자(Relying Party) 역할을 동시에 하는 OAuth 2.0 유저에이전트 + 소셜 로그인 플로우 시퀀스 다이어그램 템플릿.
