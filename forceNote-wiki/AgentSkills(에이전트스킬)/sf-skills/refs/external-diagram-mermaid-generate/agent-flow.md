---
tags: [agent-skill, sf-skills, reference, diagram, mermaid, agentforce]
source: forcedotcom/sf-skills (skills/external-diagram-mermaid-generate/assets/agentforce/agent-flow.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Agentforce Flow Diagram Template, Agentforce 에이전트 플로우 다이어그램, agent topics actions conversation flow]
---

# Agentforce Flow Diagram Template — Agentforce 에이전트 플로우 다이어그램 템플릿

> Agentforce 에이전트 구조(에이전트·토픽·액션·인스트럭션)와 대화 흐름(conversation flow)을 Mermaid flowchart로 시각화하는 템플릿.
