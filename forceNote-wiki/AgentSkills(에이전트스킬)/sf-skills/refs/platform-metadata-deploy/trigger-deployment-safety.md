---
tags: [agent-skill, sf-skills, reference, platform, metadata, trigger]
source: forcedotcom/sf-skills (skills/platform-metadata-deploy/references/trigger-deployment-safety.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Trigger Deployment Safety, cascade failure, savepoint, async decoupling, 트리거 배포 안전, 캐스케이드 실패, 원자성]
---
# Trigger Deployment Safety Guide — 트리거 배포 안전 가이드

> 트리거 캐스케이드 실패, Savepoint 기반 원자성 패턴, Queueable/Platform Event/@future 비동기 디커플링, 배포 전 체크리스트와 캐스케이드 시나리오 테스트.
