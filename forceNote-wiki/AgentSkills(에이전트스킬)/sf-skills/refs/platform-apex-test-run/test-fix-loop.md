---
tags: [agent-skill, sf-skills, reference, platform, apex-test, automation]
source: forcedotcom/sf-skills (skills/platform-apex-test-run/references/test-fix-loop.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Agentic Test-Fix Loop, 테스트 자동 수정 루프, failure analysis decision tree, 실패 분석]
---
# Agentic Test-Fix Loop Implementation — 에이전트 기반 테스트-수정 루프

> platform-apex-test-run의 자동 수정 루프: 테스트 실행 → 실패 파싱 → 소스 분석 → sf-apex로 수정 생성 → 재실행을 최대 3회 반복하는 방식.

## How It Works

When the agentic loop is enabled, platform-apex-test-run will:
...
- [[platform-apex-test-run/cli-commands|cli-commands]]
- [[test-patterns]]
