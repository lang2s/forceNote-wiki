---
tags: [agent-skill, sf-skills, reference, dx, code-analyzer, config-schema]
source: forcedotcom/sf-skills (skills/dx-code-analyzer-configure/references/config-schema.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Config Schema, 설정 스키마, code-analyzer.yml, 엔진 설정, suppressions, 룰 오버라이드]
---
# Code Analyzer Configuration Schema — code-analyzer.yml 설정 스키마

> `code-analyzer.yml` 설정 파일의 전체 레퍼런스 — 최상위 필드, ignores, rules 오버라이드, 엔진별(PMD/ESLint/CPD/SFGE/ApexGuru/Flow/Regex/RetireJS) 설정, suppressions, 설정 파일 발견 순서, 검증 명령어.
