---
tags: [agent-skill, sf-skills, reference, slds, design-system, linter-rule]
source: forcedotcom/sf-skills (skills/design-systems-slds2-migrate/references/rule-no-hardcoded-values.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Rule No Hardcoded Values, no-hardcoded-values-slds2 룰, 하드코딩 값 교체, decision tree, utility class]
---

# Rule: No Hardcoded Values — `slds/no-hardcoded-values-slds2` 룰

> SLDS 2 훅 대응이 있는 하드코딩 값(색상·spacing·sizing·typography·border·radius·shadow)을 훅으로 교체하는 린터 룰. 유틸리티 클래스 우선 → 1:1 훅 매핑 → 컨텍스트 선택 의사결정 트리.
...
- [[non-color-hooks-decision-guide]]
