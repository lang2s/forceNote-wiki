...
source: forcedotcom/sf-skills (skills/design-systems-slds2-migrate/references/rule-no-slds-class-overrides.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Rule No SLDS Class Overrides, no-slds-class-overrides 룰, SLDS 클래스 오버라이드 금지, 네임스페이스 클래스]
---

# Rule: No SLDS Class Overrides — `slds/no-slds-class-overrides` 룰

> `.slds-*` 클래스를 직접 오버라이드하는 CSS 셀렉터를 탐지하고, `{componentName}-{sldsElementPart}` 네임스페이스 클래스로 분리하는 린터 룰. 마크업에서 원본 SLDS 클래스는 절대 제거하지 않고 병기한다.
