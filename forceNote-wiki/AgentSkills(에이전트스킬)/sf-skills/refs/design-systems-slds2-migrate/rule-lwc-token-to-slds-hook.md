...
source: forcedotcom/sf-skills (skills/design-systems-slds2-migrate/references/rule-lwc-token-to-slds-hook.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Rule LWC Token to SLDS Hook, lwc-token-to-slds-hook 룰, --lwc 토큰 교체, surface vs container]
---

# Rule: LWC Token to SLDS Hook — `slds/lwc-token-to-slds-hook` 룰

> deprecated `--lwc-*` 디자인 토큰을 SLDS 2 스타일 훅으로 교체하는 린터 룰. 린터가 제안한 훅만 사용하고, 다중 제안일 때는 surface vs container 패턴 매칭으로 선택한다.
