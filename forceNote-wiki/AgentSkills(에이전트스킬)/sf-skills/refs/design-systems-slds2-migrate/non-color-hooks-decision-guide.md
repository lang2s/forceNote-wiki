---
tags: [agent-skill, sf-skills, reference, slds, design-system, non-color-hooks]
source: forcedotcom/sf-skills (skills/design-systems-slds2-migrate/references/non-color-hooks-decision-guide.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Non-Color Styling Hooks Guide, 비색상 스타일 훅 가이드, spacing sizing typography border shadow hooks]
---

# Non-Color Styling Hooks Guide — 비색상 스타일 훅 가이드

> 하드코딩된 spacing·sizing·typography·border·radius·shadow 값을 SLDS 2의 번호 스케일 스타일 훅으로 교체하는 레퍼런스. 색상 훅과 달리 컨텍스트 판단이 거의 필요 없는 직접 매핑.
