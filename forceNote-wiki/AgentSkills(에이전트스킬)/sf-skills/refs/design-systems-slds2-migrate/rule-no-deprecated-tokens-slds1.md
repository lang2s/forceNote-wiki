...
source: forcedotcom/sf-skills (skills/design-systems-slds2-migrate/references/rule-no-deprecated-tokens-slds1.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Rule No Deprecated Tokens SLDS1, no-deprecated-tokens-slds1 룰, Aura t() 토큰 교체, token mappings]
---

# Rule: No Deprecated Tokens SLDS1 — `slds/no-deprecated-tokens-slds1` 룰

> 레거시 Aura `t(tokenName)` / `token(tokenName)` 구문을 SLDS 2 스타일 훅으로 교체하는 린터 룰. 텍스트/배경/spacing/radius/폰트 토큰 매핑표와 마법사 토큰 매핑을 포함.
