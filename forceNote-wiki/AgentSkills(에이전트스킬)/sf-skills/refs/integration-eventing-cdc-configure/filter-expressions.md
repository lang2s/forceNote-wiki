...
source: forcedotcom/sf-skills (skills/integration-eventing-cdc-configure/references/filter-expressions.md, 공식 Salesforce)
created: 2026-06-27
aliases: [CDC Filter Expression Reference, CDC 필터 표현식, filterExpression SOQL, 채널 필터 연산자]
---

# Filter Expression Reference — CDC 필터 표현식 레퍼런스

> CDC `<filterExpression>`(WHERE 절 본문)에서 지원되는 필드 타입별 연산자·복합식·null 체크·함수·복합 필드(Address)·미지원 패턴.
