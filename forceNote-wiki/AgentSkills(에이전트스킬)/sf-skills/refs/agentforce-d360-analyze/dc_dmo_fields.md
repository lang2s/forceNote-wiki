---
tags: [agent-skill, sf-skills, reference, agentforce, d360-dmo-fields]
source: forcedotcom/sf-skills (skills/agentforce-d360-analyze/references/dc_dmo_fields.md, 공식 Salesforce)
created: 2026-06-27
aliases: [STDM GenAI DMO field reference, DMO 필드 레퍼런스, 크로스 DMO 조인맵, GenAIGatewayRequest, 24 쿼리 워터폴]
---

# STDM + GenAI DMO field reference — STDM·GenAI DMO 필드 레퍼런스

> 이 스킬이 다루는 DMO들의 필드 레퍼런스 — 케이싱 함정, 크로스 DMO 포워드 조인맵, 24개 쿼리 워터폴, DMO 머터리얼라이즈 타이밍, Session Tracing(5)·Generative AI Audit & Feedback(12)·Agent Optimization(6)·Platform Tracing(1) DMO 스키마, 검증된 enum 값까지 망라.
