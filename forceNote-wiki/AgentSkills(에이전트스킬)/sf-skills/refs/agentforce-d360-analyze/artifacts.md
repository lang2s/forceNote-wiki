---
tags: [agent-skill, sf-skills, reference, agentforce, d360-artifacts]
source: forcedotcom/sf-skills (skills/agentforce-d360-analyze/references/artifacts.md, 공식 Salesforce)
created: 2026-06-27
aliases: [D360 Artifacts, 아티팩트 레퍼런스, dc 세션 트리, session summary, 읽기 순서]
---

# Artifacts reference — D360 아티팩트 레퍼런스

> 단일 Agentforce 세션 트레이스가 디스크에 떨어뜨리는 `dc.*` 아티팩트 목록(생성 순서)과, 분석 시 읽어야 할 권장 순서(summary → tree → manifest → 원시 DMO JSON)를 정리한 레퍼런스.
