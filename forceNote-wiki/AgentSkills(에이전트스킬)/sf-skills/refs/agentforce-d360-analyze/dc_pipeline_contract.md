---
tags: [agent-skill, sf-skills, reference, agentforce, d360-pipeline]
source: forcedotcom/sf-skills (skills/agentforce-d360-analyze/references/dc_pipeline_contract.md, 공식 Salesforce)
created: 2026-06-27
aliases: [DC Pipeline Contract, DC 파이프라인 계약, fetch assemble render, session_tree, binding_method]
---

# DC Pipeline Contract — DC 파이프라인 계약

> 단일 Agentforce 세션의 DC 파이프라인 3단계(fetch → assemble → render) 계약 — 어셈블 단계의 트리 셰이프·바인딩 체인·타임스탬프 윈도우 폴백·`binding_method`/`session_shape` enum·`_session_tree.json` 스키마, 렌더 단계의 섹션 순서·생략 규칙·세션-종료 유도·멱등성을 규정.
