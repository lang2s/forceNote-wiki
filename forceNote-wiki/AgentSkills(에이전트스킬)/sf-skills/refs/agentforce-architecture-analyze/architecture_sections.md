---
tags: [agent-skill, sf-skills, reference, agentforce, architecture-sections]
source: forcedotcom/sf-skills (skills/agentforce-architecture-analyze/references/architecture_sections.md, 공식 Salesforce)
created: 2026-06-27
aliases: [architecture.md section reference, 아키텍처 섹션 레퍼런스, render_architecture, mermaid 노드 캡, 액션 트리]
---

# architecture.md section reference — 아키텍처 섹션 레퍼런스

> `scripts/render_architecture.py::render`가 생성하는 `architecture.md`의 섹션별 렌더링 스펙 — 다이어그램 노드 캡, 7개 번호 섹션(헤더·해부 요약·액션 트리·토픽 해부·액션 카탈로그·데이터 흐름·Flow/Apex/Prompt 카탈로그·미해결 참조)과 조건부 의존성 그래프 부록.
...
```text
// 구조 예시 — render_architecture.py가 생성하는 architecture.md 의 헤딩 골격
# Architecture
...
## Dependency graph        # conditional appendix
```
...
(backtick-wrapped) block per topic + kv list (`- Label:`, `- Action count:`, `- Actions:`
with sub-bullets).
...
column renders as `KIND api_name` (backtick-wrapped) when `unwraps_to` is present, `-`
...
templates` H3 buckets, each containing `#### <api_name>` (backtick-wrapped) + a fenced
