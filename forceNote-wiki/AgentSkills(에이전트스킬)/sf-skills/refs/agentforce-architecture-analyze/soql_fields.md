---
tags: [agent-skill, sf-skills, reference, agentforce, soql-fields]
source: forcedotcom/sf-skills (skills/agentforce-architecture-analyze/references/soql_fields.md, 공식 Salesforce)
created: 2026-06-27
aliases: [sObject field reference architecture, sObject 필드 레퍼런스, GenAiPlannerDefinition, Tooling API, 크로스 sObject 조인맵]
---

# sObject field reference — architecture skill (sObject 필드 레퍼런스)

> 아키텍처 스킬이 15개 SOQL 템플릿으로 조회하는 13개 sObject의 필드 레퍼런스 — 케이싱 함정, complexvalue 단일행 제약, 크로스 sObject 포워드 조인맵, Data API(2)·Tooling API(11) sObject 스키마, 검증된 picklist 값, mandatory 필드 강제.
