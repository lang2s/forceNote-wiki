---
tags: [agent-skill, sf-skills, reference, agentforce, improve]
source: forcedotcom/sf-skills (skills/agentforce-observe/references/improve-reference.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Improve Phase, 개선 단계, agent 파일 편집, 회귀 방지, 안전 재검증]
---

# Phase 3: Improve — `.agent` 파일 편집 풀 레퍼런스

> Phase 3에서 Edit 도구로 `.agent` 파일을 직접 수정한다 — 액션 타깃 가용성 사전 점검, `.agent` 구조와 메타데이터 매핑, 이슈→수정 위치 매핑, 효과적 서브에이전트 지시 원칙, 회귀 방지, 배포/검증, 안전 재검증, Testing Center 테스트 케이스 갱신을 다루는 레퍼런스.
