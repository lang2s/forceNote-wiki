---
tags: [agent-skill, sf-skills, reference, agentforce, stdm-queries]
source: forcedotcom/sf-skills (skills/agentforce-observe/references/stdm-queries.md, 공식 Salesforce)
created: 2026-06-27
aliases: [STDM Query Reference, STDM 쿼리, AgentforceOptimizeService, 세션 조회, 집계 메트릭]
---

# STDM Query Reference — STDM 쿼리 레퍼런스

> `AgentforceOptimizeService` Apex 헬퍼 클래스로 STDM DMO를 조회하는 절차 — 클래스 배포, 세션 검색, 대화 상세, LLM 프롬프트/응답, 집계 메트릭, 모먼트 인사이트, 관측성(RAG) 쿼리, 대화 재구성.
