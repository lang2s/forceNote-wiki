---
tags: [agent-skill, sf-skills, reference, dx, code-analyzer, post-scan-workflows]
source: forcedotcom/sf-skills (skills/dx-code-analyzer-run/references/post-scan-workflows.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Post-Scan Workflows, 스캔 후 워크플로우, 결과 쿼리, 룰 설명, 룰 목록, query-results, describe-rule]
---
# Post-Scan Workflows — 스캔 후 워크플로우 (결과 쿼리·룰 설명·룰 목록)

> 초기 스캔 결과 제시(Step 5) 이후 사용자가 후속 질문을 할 때의 세 가지 워크플로우 — 결과 재필터링(Result Querying), 룰 설명(Rule Description), 룰 목록(Rule Listing) — 의 스크립트 사용법·출력 포맷·제시 규칙.
