---
tags: [agent-skill, sf-skills, reference, dx, code-analyzer, vendor-file-handling]
source: forcedotcom/sf-skills (skills/dx-code-analyzer-run/references/vendor-file-handling.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Vendor File Handling, 벤더 파일 처리, 서드파티 라이브러리, intelligent filter, jQuery 위반]
---
# Vendor File Handling — 벤더(서드파티) 파일 처리

> jQuery·Bootstrap 등 서드파티 라이브러리가 만들어내는 대량 위반을 프로젝트 소스와 분리하는 두 가지 방법(--target 재스캔, 지능형 필터)과 다중 휴리스틱 신뢰도 점수 시스템 설명.
