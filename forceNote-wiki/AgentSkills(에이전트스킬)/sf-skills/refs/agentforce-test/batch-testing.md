---
tags: [agent-skill, sf-skills, reference, agentforce, testing-center]
source: forcedotcom/sf-skills (skills/agentforce-test/references/batch-testing.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Testing Center Batch Testing, 테스팅 센터 배치 테스트, sf agent test, AiEvaluationDefinition]
---

# Mode B: Testing Center Batch Testing — 테스팅 센터 배치 테스트 풀 레퍼런스

> Salesforce 내장 Agentforce 테스트 인프라(Testing Center)로 테스트 스펙 YAML 작성 → 배포 → 실행 → 결과 분석 → 수정 루프까지 다루는 전체 레퍼런스(토픽 이름 해석·Level 1/2 액션명·알려진 버그 포함).
