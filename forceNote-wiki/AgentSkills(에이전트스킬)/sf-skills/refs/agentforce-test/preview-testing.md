---
tags: [agent-skill, sf-skills, reference, agentforce, preview-testing]
source: forcedotcom/sf-skills (skills/agentforce-test/references/preview-testing.md, 공식 Salesforce)
created: 2026-06-27
aliases: [Ad-Hoc Preview Testing, 프리뷰 테스트, sf agent preview, 트레이스 분석, safety probe]
---

# Mode A: Ad-Hoc Preview Testing — 애드혹 프리뷰 테스트 풀 레퍼런스

> `sf agent preview`로 로컬 `.agent` 번들을 컴파일해 테스트 케이스 계획 → 프리뷰 실행 → 트레이스 8축 분석 → 자동 수정 루프까지 다루는 전체 레퍼런스(안전 프로브·세이프티 판정 포함).
