...
> 8개 도메인 서브폴더로 분류됨(STRUCT-2). 각 섹션 헤더의 서브폴더 index 링크로 로컬 탐색.

🗺️ **허브(루트 잔류):** [[Salesforce 어드민 종합 개요]] — 어드민 전체 지도(8개 도메인 진입점). 무엇부터 볼지 여기서 시작. `#overview #hub`

### 조직 설정 (서브폴더 → [[Admin(어드민)/OrgSetup(조직설정)/index]])
...
### 사용자·접근 (서브폴더 → [[Admin(어드민)/Users(사용자·접근)/index]])
...
### 로그인·세션보안 (서브폴더 → [[Admin(어드민)/LoginSecurity(로그인·세션보안)/index]])
...
### 데이터관리 (서브폴더 → [[Admin(어드민)/Data(데이터관리)/index]])
...
### 오브젝트·필드 (서브폴더 → [[Admin(어드민)/ObjectsFields(오브젝트·필드)/index]])
...
### 사용자인터페이스 (서브폴더 → [[Admin(어드민)/UI(사용자인터페이스)/index]])
...
### 자동화 (서브폴더 → [[Admin(어드민)/Automation(자동화)/index]])
...
### 이메일·분석 (서브폴더 → [[Admin(어드민)/EmailAnalytics(이메일·분석)/index]])
