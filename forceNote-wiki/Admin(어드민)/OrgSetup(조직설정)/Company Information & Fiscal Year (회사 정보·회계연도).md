...
| 구분 | Standard | Custom |
|---|---|---|
| 기준 | Gregorian 달력 | 비-Gregorian(주 단위) 구조 |
| 조정 범위 | **시작 월(start month)** 만 변경 | **분기·기간(period)을 주(week) 단위로** 정의 |
| 대상 | 일반 조직 | 소매·제조 등 회계 기간이 달력과 다른 조직 |

**4-4-5 등 주간 구조:** 커스텀 회계연도는 기간(period)을 **주(week) 단위**로 짠다. 대표 템플릿인 **4-4-5**는 한 분기를 3개 기간으로 나누고 주 수를 **4주 – 4주 – 5주**로 배분한다. 이 밖에 **13-period**(4주짜리 기간 13개) 같은 공통 템플릿도 제공되어, 시작일과 템플릿만 지정하면 빠르게 정의할 수 있다. (그 외 변형 템플릿·주 배분 세부는 공식 문서 참조.)

**활성화 후 비가역(중요):** 커스텀 회계연도를 **enable 하면 다시 disable 할 수 없다** — 표준 회계연도로 되돌릴 수 없는 **영구 결정**이다(리포트·예측·쿼터 등 날짜 기반 데이터 전반에 광범위하게 영향을 주기 때문). 표준 구조로 돌아가려면 disable 이 아니라, **Gregorian 표준과 동일한 구조의 커스텀 회계연도를 다시 정의**하는 우회만 가능하다.

> 출처: Salesforce Help — help.salesforce.com (About Custom Fiscal Years `sf.admin_about_cfy.htm` / Choose a Custom Fiscal Year Template). 4-4-5·13-period·비가역 서술은 공식 문서 기준(접속 2026-07-12).
> 커스텀 회계연도의 나머지 분기·기간 세부 설정은 이 노트 범위 밖 — 공식 문서(위 `official_doc`) 참조.
