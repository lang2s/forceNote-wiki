...
# NAV_MAP — 폴더 로컬 인덱스 (Layer 3) 지도

> 각 subfolder의 로컬 index.md 위치 지도. Layer 3 탐색 진입점. (구 CLAUDE.md "폴더 로컬 인덱스" 표에서 이전 — index-manager 단일 소유)
...
| Admin(어드민)/OrgSetup(조직설정) | `Admin(어드민)/OrgSetup(조직설정)/index.md` |
| Admin(어드민)/Users(사용자·접근) | `Admin(어드민)/Users(사용자·접근)/index.md` |
| Admin(어드민)/LoginSecurity(로그인·세션보안) | `Admin(어드민)/LoginSecurity(로그인·세션보안)/index.md` |
| Admin(어드민)/Data(데이터관리) | `Admin(어드민)/Data(데이터관리)/index.md` |
| Admin(어드민)/ObjectsFields(오브젝트·필드) | `Admin(어드민)/ObjectsFields(오브젝트·필드)/index.md` |
| Admin(어드민)/UI(사용자인터페이스) | `Admin(어드민)/UI(사용자인터페이스)/index.md` |
| Admin(어드민)/Automation(자동화) | `Admin(어드민)/Automation(자동화)/index.md` |
| Admin(어드민)/EmailAnalytics(이메일·분석) | `Admin(어드민)/EmailAnalytics(이메일·분석)/index.md` |
| DevOps(데브옵스) | `DevOps(데브옵스)/index.md` |
| DevOps(데브옵스)/DX(DX개발환경) | `DevOps(데브옵스)/DX(DX개발환경)/index.md` |
| DevOps(데브옵스)/Deploy(배포·CICD) | `DevOps(데브옵스)/Deploy(배포·CICD)/index.md` |
| DevOps(데브옵스)/Packaging2GP(2세대패키징) | `DevOps(데브옵스)/Packaging2GP(2세대패키징)/index.md` |
...
| Service(서비스)/Chat(채팅) | `Service(서비스)/Chat(채팅)/index.md` |
| Service(서비스)/Knowledge(지식) | `Service(서비스)/Knowledge(지식)/index.md` |
...
- [[00 SEARCH_INDEX]] — 키워드 라우터 (Layer 1, 도메인 → 샤드)
- [[00 Home]] — 전체 진입점 (Layer 0)
