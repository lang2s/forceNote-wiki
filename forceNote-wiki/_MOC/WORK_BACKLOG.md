...
| RECON-17b | `exp_cloud_lwr.pdf` Ch3 — **LWR 컴포넌트 개발 심화** spoke | 🔲 대기 — Create Site/Pages/Theme Layouts·Create Components(js-meta.xml 전수)·Component Properties·Base Component Limitations·@salesforce Modules·반응형(`--dxp-c-screensize` 코드)·Custom Layout/Nav Menu 컴포넌트(코드 다수)·Publish. 주 노트 "컴포넌트 개발" H2의 deep dive. 제안 `LWC/UIPatterns(UI패턴)/LWR 컴포넌트 개발 심화.md` | **P3** | 🔲 대기 | 2026-06-20 |
| RECON-17c | `exp_cloud_lwr.pdf` Ch4 — **LWR `--dxp` 스타일링 훅 레퍼런스** spoke | 🔲 대기 — ⚠️ **8페이지 거대 다단 매핑 표**(Theme panel 속성↔`--dxp` 훅, Colors/Text/Spacing 카테고리). How Branding Works·Enable·Color/Text/Spacing 훅 전수·Custom Components 사용·Override CSS·Color Palette·Site Logo·Custom Fonts·Remove SLDS. **pdftoppm 이미지화 권장**(매핑 표 collapse 위험). 주 노트 브랜딩 H2의 lookup spoke. 제안 `LWC/UIPatterns(UI패턴)/LWR --dxp 스타일링 훅 레퍼런스.md` | **P3** | 🔲 대기 | 2026-06-20 |
