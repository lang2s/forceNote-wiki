...
| LINT-ELASTIC | `Apex/ExecutionContext(실행컨텍스트)/Governor Limits.md` Platform Apex 한도 표에 **`DailyAsyncApexElasticExecutions`(elastic executions beta) 1행 보강** | ING-29 classifier가 app_limits 치트시트(p.5-6) 대조 중 발견 — 기존 Governor Limits.md Platform Apex 표에 이 한도(일일 비동기 한도 + min(licensed daily, 10M) elastic 추가분, 최대 10M)가 누락. 소관이 거버너 한도라 ING-29 신규 노트가 아닌 기존 노트 보강 대상. **✅ 완료(2026-06-22)** — `pdftotext -layout`로 원문 셀 재추출(컬럼 collapse 우회), elastic 행 추가 + 기존 `DailyAsyncApexExecutions` 행에 org 한도명 보강(parity). source frontmatter에 app_limits_cheatsheet 추가(Tier 2). | ✅ 완료 | 2026-06-20 |
| LINT-BOM | `Apex/SOQL(SOQL)/SOQL 문법 레퍼런스.md` 1행 **UTF-8 BOM(`ef bb bf`) 제거** + `_index/*.md` 일괄 BOM 점검 | ING-29 cross-linker가 역링크 편집 중 발견(lint false positive 유발). 별건 정비. wiki-linter에 BOM 검사 추가 권고. **✅ 완료(2026-06-22)** — BOM 점검 결과 **2건** 발견(예상 1건보다 많음): SOQL 노트 + `_index/sobject-reference.md` 샤드. 둘 다 `perl -pe s/^\xEF\xBB\xBF//`로 제거·검증. wiki-linter BOM 검사 추가는 wiki-retrospective 권고로 잔존. | ✅ 완료 | 2026-06-20 |
...
| ING-19 | ~~service_presence_developer_guide (23)~~ / _administrators (124) | Service (Omni-Channel) | **✅ dev guide 완료 (2026-06-20)** → [[WORK_BACKLOG_ARCHIVE]] ING-19. ★ 파일명 함정: `service_presence`지만 실제 "**Omni-Channel Developer Guide**" v67.0. ★ **Standard Omni-Channel EOL Summer '26** — 은퇴 배너. 신규 `Service(서비스)/OmniChannel(옴니채널)/` 폴더 **2노트**: `Omni-Channel 객체·메타데이터·콘솔 컴포넌트`(215줄, 24객체·11메타타입·콘솔 메서드 12+10·이벤트 10·표 4개) + `Omni-Channel External Routing`(499줄, Pub/Sub XML 4·Apex trigger 3·CDC 모니터링·Expected Behavior 5시나리오·Troubleshooting). 중복회피: Console JS API(ING-24)·Service Cloud Objects deep-link. 원문 불일치 각주 2건(`__chn`/`_chn`·`AgentWorkPresenceChangeEvent`/`AgentWorkChangeEvent`). 8검증 갭0/불일치0·qa PASS. **잔여: `_administrators` 124p admin 가이드(ING-30)** | **(admin 잔여)** | ✅ dev 완료 | 2026-06-20 |
...
| ING-21 | ~~salesforce_guided_engagement (40)~~ | Service | **✅ 완료 (2026-06-20)** → [[WORK_BACKLOG_ARCHIVE]] ING-21. ★ 파일명 함정: `guided_engagement`는 레거시명, 실제 "**Lightning Flow for Service**" — 핵심 **Actions & Recommendations 컴포넌트**+**RecordAction junction 객체**. 백로그 가정 용어(deploymentChannel·Guided Action List)는 PDF 부재→fabricate 금지. 신규 `Service(서비스)/Lightning Flow for Service (Actions & Recommendations).md`(451줄, 8챕터: RecordAction·deployment·Enhance 7옵션·Process Builder 필드표·Apex 예제 VERBATIM·권한표 5종 BLANK 보존·Chat/Open CTI 통합). 위임 多(Flow·NBA·Open CTI·Big object·Packaging 8 deep-link). 8검증 갭0/불일치0·qa PASS | **P3** | ✅ 완료 | 2026-06-20 |
| ING-23 | ~~salesforce_mobile_push_notifications_implementation (57)~~ | Mobile/Notification | **✅ 완료 (2026-06-20)** → [[WORK_BACKLOG_ARCHIVE]] ING-23. = "Salesforce Mobile Notifications Implementation Guide" Summer '26. 신규 `Apex/Messaging(메시징)/Mobile Notifications.md`(614줄): 2시스템(Notification Builder vs Apex push)·`Messaging.PushNotification`(생성자2·메서드3)·`PushNotificationPayload`(apple 4+8-param)·customNotificationAction invocable·Connect REST·Notifications Resources·Apex Limits 함수 2·Debug Log 6종. ★ 동명이의 구분: customNotificationAction(invocable REST) ≠ Messaging.CustomNotification(Apex). 한도 [[Governor Limits]] 위임. 역링크: Messaging Namespace→상세 본거지. 8검증 갭0/불일치0·qa PASS | ✅ 완료 | 2026-06-20 |
...
| ING-33 | ~~order_management_developer_guide_html (69)~~ | Commerce/OM | **✅ 완료 (2026-06-20)** → [[WORK_BACKLOG_ARCHIVE]] ING-33. **위키 첫 `Commerce(커머스)/` 폴더 신설 + 4노트**: `Order Management 개요와 데이터 모델`(허브, 138줄)·`B2C Commerce Storefront Order Data Map`(671줄, **21객체 매핑표 225행**)·`Order Management — Import·Fulfillment·Taxation`(362줄)·`Order Management — Exchanges·Payment Sequencing·확장`(618줄, Apex 예제 2종·ProductExpandService). ★ 69p 중형(33p 데이터맵)이라 전용 패스·3 researcher 병렬·4노트 분할. 중복 회피: ConnectApi/CommercePayments 네임스페이스 deep-link(메서드 재작성 0). 다이어그램 추출 불가(pdftoppm 실패)→추측 금지. 원문 오타 다수 보존+마커. 8검증(4completeness+4verifier) 전부 갭0·불일치0·transpose0·qa PASS. Tier3 0. nav: 신규 Commerce index·apex-namespaces 샤드 4행·questions 3행·Home Commerce 섹션. cross-linker: 형제 링크 복원(병렬작성 plain-text 폴백)+CommercePayments/CommerceExtension 역링크 |
