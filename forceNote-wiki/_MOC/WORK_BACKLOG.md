...
|---|---|---|---|---|---|
| STRUCT-2 | **Admin(어드민) 8도메인 서브폴더화** | 직속 71노트 = 위키 최대 평탄 폴더. 허브 노트 `Salesforce 어드민 종합 개요`가 이미 8도메인(조직설정·사용자/접근·보안·데이터·오브젝트/필드·UI·자동화·이메일)으로 분류해 둔 논리 구조를 폴더가 못 따라감. Obsidian wikilink는 파일명 기준이라 **폴더 이동은 링크 무손상** — 비용은 `_index/admin.md` 경로 ~71행 + index.md 재구성. **다음 Admin 대형 작업과 묶어 수행**(단독 커밋은 효익 낮음). 실행 주체: pm 지휘 + index-manager(경로) — SEAM_MAP "파일 이동" RACI 행 신설됨 | 탐색성·폴더 일관성 | 🔲 대기 | 2026-07-17 |
| STRUCT-3 | **DevOps(데브옵스) 서브폴더화 (2GP·DX 군집)** | 직속 64노트, 파일명 접두어로 군집 뚜렷: `2GP — *`(≈24)·DX/Scratch Org(≈13)·Unlocked Package(5). MetadataAPI 등 서브폴더 3개 선례 있음. STRUCT-2와 동일 방법·후순위 | 탐색성 | 🔲 대기 | 2026-07-17 |
...
> ✅ **STRUCT-1·4·5·6·8·9 완료 (2026-07-18)** → [[WORK_BACKLOG_ARCHIVE]] "STRUCT·PIPE·DEC" 섹션. 위 표에 남은 STRUCT-2·3·7은 **설계상 지연**(다음 대형작업/다음 Apex 이니셔티브와 묶어 수행 — 단독 실행 효익 낮음).
