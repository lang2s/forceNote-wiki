...
| ING-26 | api_tooling (1006, 분할) = **Tooling API Reference & Developer Guide v67.0 Summer '26** | Dev Tools | 🟡 진행중 — 신규 폴더 `DevOps(데브옵스)/ToolingAPI(툴링API)/`. 계획: 콘텐츠 14노트(N1~N3 + Ch4 도메인 11: C4-1~8 + C4-9a/b/c) / 사이클 15. 오프셋 물리=인쇄+10. Ch4 객체 295(DELEGATE 14 제외 신규 281). **경계구역(N1~N3) ✅ 완료**: Cycle 1 N1 개요·REST·SOAP(1159줄, d44c446) · Cycle 2 N2 Objects&Namespaces(346줄, 87객체·Setup 57 PDF경계 직접대조) · Cycle 3 N3 SOAP·REST 헤더(426줄, SOAP 8·REST 4·enum 6/9/8, AP-09 3페이지 샘플 완결). 전부 completeness/source-verifier 갭0·불일치0·qa PASS. 정본 계획/로스터: scratchpad `ING-26-plan.md`·`ING-26-ch4-roster.md`. 메모리 [[project_tooling_api_ingest]]. **Ch4 진행: C4-1·C4-6·C4-5·C4-3·C4-4·C4-2·C4-7 ✅ (7/11)**. C4-1 Apex 17(657줄). C4-6 Entity·Field 28(1044줄). C4-5 보안·권한 38+3서브(1385줄, 4dba7a2). C4-3 자동화 19(622줄, 55f46a9). C4-4 UI·레이아웃 22(899줄, 2a88873). C4-2 Lightning 5(245줄, 97c08c4). C4-7 운영/패키징 **2노트 38객체**(5ae267a — A 운영·라이프사이클 18객체 636줄[Sandbox·Deploy·ReleaseUpdate·SourceMember·MyDomain·BizProcess 잔여3] + B 패키징·브랜딩 20객체 956줄[1GP/2GP/구독자 패키지·BrandingSet·StaticResource]; 계획~32 과소→실측41-기작성3=38; SandboxProcess.Status·InstallValidationStatus 각17값; fabricate 금지 Package2Version.ReleaseState·Package2.PackageType 부재). 전부 갭0·불일치0·qa PASS. **남은 Ch4 3그룹(4노트)** 권고순: **C4-8 User/이벤트(~18,+이연 User)→C4-9a/b/c(35/24/22,9a+이연 EmbeddedService* 11종·SvcCatalogFulfill*,9b+RecommendationStrategy,9c+이연 UserCriteria·ModerationRule)**. ★계획 객체수 추정치—scout 실측 우선(C4-3 27→19·C4-4 32→22·C4-2 5→5·C4-7 32→41, 양방향 빗나감). **★★매 사이클 기작성 객체 grep 제외 필수**(C4-7서 Certificate·IconDefinition·ProcessFlowMigration 3종 중복 발견·제외). 로스터 scratchpad 휘발→그룹별 scout 재생성 |
