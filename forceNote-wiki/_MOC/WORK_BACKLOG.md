...
### 🟠 W27 — Winter '27 (v68.0) 릴리즈 후속 · 의도적 스코프 아웃 (2026-08-25 신설)
...
> **재추출 레시피(롱테일 공통):** page id → `https://help.salesforce.com/s/articleView?id=release-notes.<page_id>.htm&language=en_US&release=264&type=5`. 제목·page id 카탈로그의 정본은 `Release/Winter '27/Clouds.md`의 `## 제목 카탈로그` 섹션.
...
|---|---|---|---|---|---|
| W27-BL-7 | **Tooling API 개요 노트의 API End-of-Life 표가 v67판** | `Tooling API — 개요·REST·SOAP 호출 기초.md`의 EOL 표를 **v68 기준으로 갱신**: 41.0–66.0 지원 / 31.0–40.0은 Summer '27부터 deprecated. **정정에 필요한 수치가 이미 확보돼 있어 수 분 작업** — 게시된 노트의 정확성 결함이라 비용 대비 가치 최고 | 🔴 **P0** (저비용·게시본 정확성) | 🔲 대기 | 2026-08-25 |
| W27-BL-3 | **Agentforce 영역 미추출 28리프** | `rn_afcc_voice` 자식 3 · `rn_agentforce_it` 9 · `rn_ai_agents_sa` 월간 12 · `rn_einstein_work_summaries` 4. `Release/Winter '27/Agentforce.md`의 **"GA 0건 · Release Update 0건" 부재 선언은 추출된 31페이지 기준**이라 이 28리프를 확인해야 확정된다 → 게시본에 **미검증 부재 선언**이 살아 있는 상태. 재추출 레시피 동일 | 🔴 **P0** (부재 선언 확정) | 🔲 대기 | 2026-08-25 |
| W27-BL-2 | **Agentforce IT Service 51페이지(`rn_it_*`)** | 롱테일 중 **가장 큰 단일 덩어리**(19 `rn_it_service_*` + 32 `rn_it_srvcs_*`). `Agentforce.md`가 이쪽을 가리키지만 **기능 상세는 위키 어디에도 없다**. 원인: 접두어가 `rn_it_*`이라 영역 분할 시 Agentforce가 아니라 **Clouds 파티션으로 떨어짐**(AP-13과 동류의 분할 부작용) → 재개 시 접두어가 아니라 **주제**로 귀속 판정할 것 | 🔴 **P0** (최대 단일 갭) | 🔲 대기 | 2026-08-25 |
| W27-BL-1 | **Clouds 롱테일 722페이지 제목-only** | Clouds 영역 988페이지 중 **266페이지만 본문 추출**, 나머지 722페이지는 `Release/Winter '27/Clouds.md`의 `## 제목 카탈로그`에 **제목 + page id만** 실려 있다. 재추출 방법은 확립됨(위 레시피). **전량 복구가 아니라 우선순위 높은 클러스터부터** 선별 복구 — 클러스터 선정(제품군별 묶기)이 첫 단계 | 🟠 P1 (대형·분할 진행) | 🔲 대기 | 2026-08-25 |
| W27-BL-5 | **한도 치트시트 판본 교체 대응** | 8월 14일 판 치트시트(15p)에서 **Apex Governor Limits 표 4종이 통째로 삭제**됨(5월 8일 판 23p에는 존재) → heap 10/25 MB·elastic 200만 캡을 치트시트로 **교차검증 불가**. ⚠️ **미수록 ≠ 반박**(값을 내리거나 지우지 말 것). 또한 `Architecture(아키텍처)/Salesforce 한도·할당량 레퍼런스 (API·Bulk·Metadata·SOQL·VF).md`의 source가 **아직 5월 8일 판** → API/Bulk/Metadata/SOQL/VF 수치를 새 판본과 대조하는 **별도 패스 필요** | 🟠 P1 (소스 드리프트) | 🔲 대기 | 2026-08-25 |
| W27-BL-8 | **ConnectApi New and Changed Classes and Enums 미추출** | Development 62페이지 범위 밖이라 `Release/Winter '27/Development.md`가 **"범위 밖"으로 명시하고 목록을 지어내지 않았다**(올바른 처리). ConnectApi 네임스페이스 노트 보완 후보 — 재개 시 릴리즈 노트가 아니라 **Apex Reference v68.0으로 클래스·메서드 전수** | 🟡 P2 | 🔲 대기 | 2026-08-25 |
| W27-BL-6 | **REST API 문서 1,506줄 감소분 확인** | v67 24,157줄 → v68 22,651줄(430p → 410p)인데 **릴리즈 노트에 언급이 없다**. 목차 대조로 **어느 섹션이 빠졌는지** 확인 필요(문서 축소인지 기능 제거인지 판정 전까지 위키 반영 금지) | 🟡 P2 | 🔲 대기 | 2026-08-25 |
...
> **다음 세션 착수 순서 권고:** `W27-BL-7`(수 분) → `W27-BL-3`(28리프·부재 선언 확정) → `W27-BL-2`(51p) → `W27-BL-1`(클러스터 선정 후 분할) → `W27-BL-5`(한도 레퍼런스 대조 패스). `W27-BL-6`·`W27-BL-8`은 사이 작업. `W27-BL-4`·`W27-BL-9`는 **10월 GA 게시 전 착수 금지**.
