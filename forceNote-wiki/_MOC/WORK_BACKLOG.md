...
| ING-36 | chat_dev_guide (61) = Live Agent/Chat (Service Cloud Chat Developer Guide, v67.0) | Service | 🔲 **P1 — ING-13a 형제 후속 (source-coverage-checker 식별)**. ING-13a(chat_rest)는 **방문자측 REST API 레퍼런스**, 이건 **에이전트/구현 가이드**(Deployment·Visualforce 컴포넌트·Live Agent 설정·deployment API JS) → 중복 아님, 보완 관계. `Service(서비스)/Chat(채팅)/` 폴더에 추가. 동일 은퇴(2026-02-14) 배너 필요. ⚠️ **Embedded Service SDK(iOS/Android)·Messaging for In-App and Web 가이드는 디스크에 없음** — 마이그레이션 대상 노트는 해당 PDF 입수(ingest acquisition) 후에야 작성 가능 |
