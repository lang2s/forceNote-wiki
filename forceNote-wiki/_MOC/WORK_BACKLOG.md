...
| AI-CLASSIC-2b | **Einstein Discovery REST 잔여 심화** — Prediction/Model/Story CRUD 엔드포인트·Get Predictions·Prediction Definition·Predict/Refresh Jobs 표현형·Prediction Builder Scorecard(impact/correlation/weight)·Forecasting 채널 매트릭스 | AI-CLASSIC-2 코어(지표)에서 분리된 잔여. **로컬 PDF `bi_dev_guide_rest_sdd.pdf` 이미 보유**(203p, Salesforce Documents/) → pdftotext 신뢰 소싱 가능(브라우저 불요). Scorecard·Forecasting은 help.salesforce.com Tier 2(별도). 니치·저우선. | ~2–4노트 | 🔲 대기 | 2026-07-18 |
| CUSTOM-BTN-1 | **Custom Buttons & Links 트리 보강** (사용자 요청 2026-07-18, 연기됨) | help.salesforce.com `platform.customize_enterprise.htm` = "Custom Buttons and Links" 부모 페이지 + 하위 6페이지(정의·**표준 버튼/탭 홈 재정의**·**샘플**·고려사항·제한사항·컴포넌트 참조). 기존 `Admin/UI/Custom Buttons & Links` 노트는 **62줄 개요뿐**(본문에 "URL·Visualforce·병합필드는 범위 밖" 명시)이라 재정의·샘플·고려·제한·병합필드가 **진짜 갭**. 소싱=help SPA 브라우저(스크린샷 기반, get_page_text는 숨은 모달 잡음 — 스크린샷 추출 권장). 사용자가 "요청 2(Formula) 먼저"로 **연기**. | ~1–2노트(기존 개요 보강) | 🔲 대기 | 2026-07-18 |
