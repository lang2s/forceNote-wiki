...
| RECON-17 | **`exp_cloud_lwr.pdf` (LWR Sites, 106p) 챕터 커버리지 — ING-10a "완료"였으나 H2 단위 대부분 미커버** | 🟡 **부분 (1차 완료 2026-06-20, 4 spoke deferred)** — ING-43(LWS) scout가 발견: ToC만 보면 위키화됨이나 H2 단위로는 완전커버 0·부분 6·미커버 2. **1차 완료**: 주 노트 `LWR Sites (Experience Cloud).md` 확장(98→285줄, Ch1 Enhanced LWR·Editions·Build Your Own vs Microsite·Ch6 비-LWS[Record Components·Apex/SOQL·Logout]·Ch8 Experience Delivery Beta) + 신규 spoke 2개(`LWR 다국어 사이트.md`[Ch5 전수·Limitations 5종]·`LWR Expressions 레퍼런스.md`[Data Binding 6행·Other 6행]). completeness 갭0·source-verifier 불일치0·qa PASS. **deferred 4 spoke(아래 RECON-17a~d). 진행: 17a✅·17b✅·17c✅(2026-06-20) → 잔여 17d**. ★ 교훈: "ING 완료" 표기도 챕터 H2 단위로 내려가면 thin slice일 수 있음(ToC 레벨 ≠ H2 레벨 커버리지) | **P3** | 🟡 부분 | 2026-06-20 |
| RECON-17a | `exp_cloud_lwr.pdf` Ch2 — **LWR 동작·캐싱·제약** spoke | **✅ 완료 (2026-06-20)** → [[WORK_BACKLOG_ARCHIVE]] RECON-17a. 신규 `LWC/UIPatterns(UI패턴)/LWR 동작·캐싱·제약.md`(260줄) — Pages/Components(Site Pages 7종·Light DOM)·New Publishing Model(Which Features 표 3행)·Custom URL(/s·vforcesite)·Caching TTL 표 4행(150d/1min/5min/1day)·Head Markup·Accessibility(F6·SPA)·**LWR Template Limitations 8하위절**(Unsupported 16항·500 routes·Dynamic Import·Sandbox asset 등). ★ 다단 표 2개 researcher가 pdftoppm 이미지로 셀 확인(AP-09 페이지경계 절단 검증). hub stub "가이드 참조"→spoke 링크 전환. completeness 갭0·fabricate0·source-verifier 불일치0·qa PASS | **P3** | ✅ 완료 | 2026-06-20 |
| RECON-17b | `exp_cloud_lwr.pdf` Ch3 — **LWR 컴포넌트 개발 심화** spoke | **✅ 완료 (2026-06-20)** → [[WORK_BACKLOG_ARCHIVE]] RECON-17b. 신규 `LWC/UIPatterns(UI패턴)/LWR 컴포넌트 개발 심화.md`(468줄) — Ch3 14절 전수: 사이트/페이지/테마레이아웃 생성·**js-meta.xml 타깃 4종**·**targetConfigs property 5종**·**@salesforce 모듈 14종 전수**·Base Component 제약·Lightning Navigation·**화면 반응형**(`--dxp-c-l/m/s-*`·64em/47.9375em)·**커스텀 레이아웃**(Page/Theme/F6)·**커스텀 내비메뉴**(Apex 인자 4종)·Publish. 중복 회피: 허브 helloSite(67.0) 복제 안 함·PDF myComponent(51.0) 정본, 브랜딩 색/텍스트 훅(Ch4=17c)·record(Ch6=허브) 제외. 원문 표기 보존 2건(`lightningCommunity_Default`·`extends Lightning Element`)+마커. completeness 14/14 갭0·source-verifier 불일치0·qa PASS. cross-linker: 허브+형제 spoke 3 역링크 | **P3** | ✅ 완료 | 2026-06-20 |
| RECON-17c | `exp_cloud_lwr.pdf` Ch4 — **LWR `--dxp` 스타일링 훅 레퍼런스** spoke | **✅ 완료 (2026-06-20)** → [[WORK_BACKLOG_ARCHIVE]] RECON-17c. 신규 `LWC/UIPatterns(UI패턴)/LWR --dxp 스타일링 훅 레퍼런스.md`(747줄) — Ch4 13절 전수: `--dxp-g/-s/-c` 훅·**Color family 30훅**·**Theme 패널 매핑표 7 카테고리**(Button Colors 3컬럼 포함, transpose0)·Override(part/data-component-id)·Color Palette·Site Logo·Custom Fonts·Remove SLDS. ⚠️ **8페이지 거대 다단 매핑표 pdftoppm r=220 이미지 셀 검증**. ★ researcher 1차 de-scope(후반 4절) 오케스트레이터 정정·보강 추출. 반응형 `--dxp-c-screensize`는 17b 위임. fabricate 금지 2건(`--dxp-c-section-*`·`@salesforce/contentAssetUrl` PDF 미정의). completeness 13/13 갭0·source-verifier 불일치0·transpose0·qa PASS. nav: frontend 샤드·LWC MOC·UIPatterns index. cross-linker: 허브+peer 2(17b·SLDS 스타일링 훅) | **P3** | ✅ 완료 | 2026-06-20 |
| RECON-17d | `exp_cloud_lwr.pdf` Ch7 — **LWR 데이터 관리 / Tag Manager 이벤트 레퍼런스** spoke | 🔲 대기 — 완전 미커버. Manage Data·Data Streams·Engagement Data·Consent Opt-In·Track to Data Cloud·**Tag Manager Event Reference 9종**(Cart·Catalog·Consent·Email·Engagement·Error Report·Line Item·Search·Wish-List 각 다단 이벤트 표). Data Cloud 도메인 경계—classifier 배치 판단. ⚠️ 9 이벤트 표 다단 셀 재확인. 제안 `LWC/UIPatterns(UI패턴)/LWR Tag Manager 데이터 관리.md` | **P3** | 🔲 대기 | 2026-06-20 |
