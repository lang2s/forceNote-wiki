...
| AI-CLASSIC-2 | **Einstein 예측형 AI 심화** — Prediction Service Apex/REST 소비·Prediction Builder Scorecard(impact/correlation/weight)·segment/example 세부·Discovery glossary 품질/편향 지표(RMSE·MAE·disparate impact·holdout)·Forecasting 채널 매트릭스 | 🟡 **부분 진행 (2026-07-18)**. **Apex 소비 절반은 인접 신규노트로 해소**: `Apex/Integration(통합)/ConnectApi CdpMachineLearning — Data 360 ML 예측.md`(로컬 Apex Reference Tier 2, 파이프라인 PASS — 단 이건 Data 360/CdpMl 계열이고 Einstein Discovery `SmartDataDiscovery.predict`와 별개). **남은 심화(Discovery 품질/편향 지표·Scorecard·Forecasting)는 여전히 미착수** — 브라우저 소싱 시도했으나 dev-SPA 본문이 get_page_text·a11y 트리에 안 잡히고 쿠키배너 미해소로 **차단**(rabbit hole 판정, 중단). **🔑 소싱 정보 확보(다음 세션 가속)**: ①정확 content ID `bi_sdd_responses_smart_data_discovery_training_metrics`(Einstein Discovery REST 가이드 `bi_dev_guide_rest_sdd`) ②**다운로드 가능 공식 PDF** `resources.docs.salesforce.com/latest/latest/en-us/sfdc/pdf/bi_dev_guide_rest_sdd.pdf`(~203p) → **PDF 입수 시 로컬 pdftotext 신뢰 소싱**으로 전환(브라우저 불요). 권장: 이 PDF를 `Salesforce Documents/`에 받아 ANALYTICS-1식 REST 표현형 전수(training metrics·prediction·models·stories). | ~1–2노트 | 🟡 진행중 | 2026-07-18 |
