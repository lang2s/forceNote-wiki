...
> **세션 인계 메모 (2026-08-25):** **W27 — Salesforce Winter '27 (v68.0) 인제스트 2단계 완료.** ①**W27-1 릴리즈 노트 위키화** — help.salesforce.com `release=264`(Tier 2, 브라우저 추출). 릴리즈 노트 전체 트리 **1,241 페이지 확보 · 그중 519 페이지를 본문까지 전수 추출**(839 KB) → 신규 6파일: `Release/Winter '27.md`(허브 179줄) + `Release/Winter '27/` 5스포크(Clouds 2,186 · Development 1,404 · Platform 679 · Agentforce 480 · Release Updates 334) 총 약 5,100줄 / 613 KB. 파이프라인: 6파티션 병렬 추출 → writer → **completeness-validator 독립 검증** → 표적 수정(5스포크 중 4개가 검증·수정 완주, Clouds 검증 진행 중). ②**W27-2 v68.0 공식 문서로 도메인 노트 심화** — `Salesforce Documents/Winter27-v68-Docs/`에 v68.0 공식 PDF 8종 확보(본문에서 `Version 68.0, Winter '27` **직접 확인**. 단 한도 치트시트는 버전 미표기). 갱신: `Governor Limits.md`(+47/−12) · `Tooling API 객체 — Apex 코드·테스트·커버리지.md`(+391, Apex Symbol API 전수) · `Tooling API — 개요·REST·SOAP 호출 기초.md`(+18/−4). **⚠️ 이번 작업의 진짜 인계 가치는 "의도적으로 스코프 아웃한 것" 9건** — 전부 아래 **🟠 W27** 섹션(W27-BL-1~9)에 사유·재개 방법·게이트와 함께 등재했다. ~~다음 세션 P0 = W27-BL-7 → W27-BL-3 → W27-BL-2~~ **(↓ 2026-08-26 갱신으로 대체 — BL-7·2는 완료 이관)**. **프로토콜 교훈은 AP-13**(영역 분할 병렬 추출 시 크로스 영역 페이지 공통 배포).

> **세션 인계 메모 갱신 (2026-08-26, wiki-retrospective 모드 B):** W27-BL **6건 완료 → [[WORK_BACKLOG_ARCHIVE]] `## 🟠 W27` 이관**(BL-1·2·5·6·7·8). BL-3은 **13/28 부분 해소**(남은 15 = AFCC Voice 3 + Service Assistant 월간 12), BL-4·9는 **10월 GA 게이트로 유지**. **AP-13 에이전트 파일 반영 완료** → `scout.md` §8 신설 + 절대 금지 3줄, `researcher.md` 크로스 영역 공통 페이지 계약 + INCOMPLETE RUN 헤더 규약. **신규 5건 등재**: W27-BL-10(v67 엔드포인트 드리프트 30파일 125곳 — ⛔일괄 sed 금지) · BL-11(Tooling 개요 예제 v67) · BL-12(`DebugLevel` 카테고리 8→11) · BL-13(샤드 상한 — `release.md` 분할은 **index-manager가 동시 세션에서 이미 실행 ✅**, 잔여는 `apex-core.md` 98.8% 임계) · **BL-14(🔴 허브·nav가 완료분과 모순 — `Winter '27.md`가 아직 "519/1,241·IT Service 51p 전부 제목만"이라 실재 본문으로 독자를 못 보냄)**. 다음 P0 = **W27-BL-14** → **W27-BL-12**(수 분) → **W27-BL-3**.
...
### 🟠 W27 — Winter '27 (v68.0) 릴리즈 후속 · 의도적 스코프 아웃 (2026-08-25 신설 · 2026-08-26 6건 완료 이관)

> **배경:** W27-1(릴리즈 노트 1,241p 중 519p 본문 추출 → 허브+5스포크) · W27-2(v68.0 공식 PDF 8종으로 도메인 노트 심화) 수행 중 **판단해서 미룬 것**의 목록. "몰라서 빠진 것"이 아니므로 **사유·재개 방법·게이트**를 행 안에 함께 남긴다 — 다음 세션이 재발견 비용을 치르지 않게 하는 것이 이 섹션의 목적.
> **✅ 2026-08-26 완료 이관 6건:** `W27-BL-1`(Clouds 롱테일 671p 전수) · `W27-BL-2`(Agentforce IT Service 51p) · `W27-BL-5`(한도 8/14판 대조) · `W27-BL-6`(REST 20p 감소 = 조판 아티팩트) · `W27-BL-7`(Tooling EOL v68) · `W27-BL-8`(ConnectApi + Work Summaries) → **[[WORK_BACKLOG_ARCHIVE]] `## 🟠 W27` 섹션**(근거·수치·교훈 보존).
> **재추출 레시피(롱테일 공통):** page id → `https://help.salesforce.com/s/articleView?id=release-notes.<page_id>.htm&language=en_US&release=264&type=5`.
> ⚠️ **정본 위치 변경(2026-08-26):** 제목·page id 카탈로그의 정본이던 `Release/Winter '27/Clouds.md`의 `## 제목 카탈로그` 절은 **롱테일 671페이지 본문 확보 완료로 함께 삭제**됐다. Clouds 영역 page id는 이제 각 클라우드 절의 항목 옆에 인라인으로 있다. **잔여 미추출(W27-BL-3)의 page id는 `Release/Winter '27/Agentforce.md`의 미추출 리프 표가 정본.**
> **⏳ 게이트 주의:** W27-BL-4·9는 **v68.0 PREVIEW 문서** 기준이라 지금 진행 불가 — 10월 GA 게시 후 재확인 항목이다(지금 쓰면 프리뷰 값·플레이스홀더를 확정 사실로 굳히게 된다). 릴리즈 노트가 선언한 값과 PREVIEW 가이드의 침묵은 **모순이 아니라 미게시**다.

| # | 항목 | 사유 / 재개 방법 | 우선순위 | 상태 | 추가일 |
|---|---|---|---|---|---|
| W27-BL-3 | **Agentforce 영역 미추출 리프 — 28 → 15로 감소** | 최초 28리프 = `rn_afcc_voice` 자식 3 · `rn_agentforce_it` 9 · `rn_ai_agents_sa` 월간 12 · `rn_einstein_work_summaries` 4. **2026-08-26 기준 13건 해소**: Work Summaries 4(→`Agentforce.md` 반영) + `rn_agentforce_it` 9(→ 하위 51리프 본문이 `Clouds.md`의 `## Agentforce IT Service` 절에 전수 확보, 허브 9축 요약은 `Agentforce.md`). **남은 15 = `rn_afcc_voice` 자식 3 + `rn_ai_agents_sa` 월간 12.** `Agentforce.md`의 "GA 0건 · Release Update 0건" 부재 선언은 여전히 **추출된 35페이지 기준**이라 이 15리프를 확인해야 확정된다 → 게시본에 **미검증 부재 선언**이 살아 있는 상태. ⚠️ `Agentforce.md` 본문의 "미추출 24" 표기는 IT Service 9 해소 **이전** 산식이므로 재개 시 15로 정정할 것 | 🔴 **P0** (부재 선언 확정) | 🟡 부분 해소 (13/28) | 2026-08-25 |
| W27-BL-14 | **🔴 허브·nav가 스포크와 모순 — W27-BL-1·2 완료분 미반영** | W27-BL-1(Clouds 988/988)·W27-BL-2(IT Service 51p 본문 확보) 이후 **상위 3곳이 옛 수치로 남아 정반대 안내를 하고 있다**(2026-08-26 실측). ①`Release/Winter '27.md` **허브** L44·L48(표 Clouds 행)·L53(합계)·L55(읽는 법) — *"519/1,241"* · *"722는 제목만"* · ***"Agentforce IT Service 51페이지(전부 제목만)"*** ← **이 문장이 가장 해롭다**: 본문이 실재하는데 독자를 원문 PDF로 되돌린다. 실제는 **Clouds 988/988 · 합계 1,241 중 약 1,241−미추출15**. ②`Release/Winter '27/index.md` L18(허브 설명 `519/1,241`)·L21(Clouds `상세 266p + 제목 카탈로그 722건`). ③~~`_index/release.md`의 *"제목 카탈로그 722건 / 미추출 제목만 / IT Service 51페이지 제목만"* 키워드 행~~ → **✅ 2026-08-26 index-manager가 동시 세션에서 제거·재작성 완료**(삭제된 절로 라우팅하던 고아 행이었다). **분담:** ①=콘텐츠 노트 → **writer/pm**(허브 `## 커버리지` 절 재작성. `Agentforce.md` 미추출 24→15 정정은 W27-BL-3과 함께 처리). ②=nav → **index-manager**(retrospective가 직접 수정하지 않음 — 권고만. ③은 이미 반영됨). ⚠️ 이 항목은 **BL-1·2의 미완 잔여물**이지 새 갭이 아니다 — *대형 갱신 후 상위 요약·nav 동기화*를 완료 조건에 넣지 않아 생겼다 | 🔴 **P0** (게시본 자기모순·독자 오도) | 🔲 대기 | 2026-08-26 |
| W27-BL-10 | **v67.0 엔드포인트 드리프트 — 30파일 125곳** | `/services/data/v67.0/...` 가 위키 **30개 파일 125곳**에 남아 있다(2026-08-26 실측. 최다: Knowledge REST 28 · Tooling 개요 15 · Scheduler 11 · REST API 9). ⛔ **일괄 치환 금지** — 대부분은 **v67 PDF를 인용한 예제**라 숫자만 바꾸면 *거짓 인용*이 되고, `Release/Spring '26/Development.md`처럼 **v67이 정답**인 경우도 있다. 올바른 처리는 **노트별 판단**: ①특정 판본 인용 예제 → 유지(+출처 판본 명시) ②버전 무관 가이드 → 공식 문서가 쓰는 `vXX.X` 플레이스홀더로 ③v67이 사실인 릴리즈 노트 → 불변. **문맥 확인 없는 `sed` 금지** | 🟠 P1 (정확성·광범위) | 🔲 대기 | 2026-08-26 |
| W27-BL-11 | **Tooling API 개요 노트 REST 예제도 v67** | `DevOps(데브옵스)/ToolingAPI(툴링API)/Tooling API — 개요·REST·SOAP 호출 기초.md`에 `v67.0` 15곳(+ composite 예제의 `v40.0`은 원문 그대로라 불변). **W27-BL-7(EOL 표 v68 갱신) 범위 밖이었다** — EOL 표만 v68이고 본문 예제 URL은 v67인 혼재 상태. **W27-BL-10의 부분집합**이므로 같은 판단 규칙을 적용하되, 이 노트는 v68 PDF가 이미 로컬에 있어(`Winter27-v68-Docs/api_tooling.pdf`) 단독 선행 처리 가능 | 🟡 P2 (BL-10 부분집합) | 🔲 대기 | 2026-08-26 |
| W27-BL-12 | **Tooling `DebugLevel` sObject — 카테고리 필드 8 → 11 (v68 신규 3)** | `Apex/Logging(로깅)/Tooling API 디버그·로그·리플레이 sObject.md`가 **v67 기준 8 카테고리**(`ApexCode`·`ApexProfiling`·`Callout`·`Database`·`System`·`Validation`·`Visualforce`·`Workflow`)로 작성돼 있다. v68에서 **`dataAccess`·`nba`·`Wave` 3개 추가 → 11개**(2026-08-26 `Winter27-v68-Docs/api_tooling.pdf` 원문 확인). ⚠️ **"8개 표에 3행 추가"로 끝나지 않는다** — 기존 8개는 `Type: picklist` + 8값이지만 신규 3개는 **`Type: ApexLogLevel (enumeration of type string)`** 이고 **`dataAccess`는 유효값이 5개뿐**(`NONE·ERROR·WARN·INFO·DEBUG` — FINE 이상 없음). 노트의 *"8개 카테고리 필드는 모두 동일한 형태"* 라는 일괄 서술이 깨지므로 **문장 재구성 필요**. 노트는 Metadata API 동명 타입 경계는 이미 반영돼 있다(v68 신규 `DebugLevel` 메타데이터 타입) | 🟠 P1 (게시본 부정확) | 🔲 대기 | 2026-08-26 |
| W27-BL-13 | **샤드 상한 — `release.md` 분할 ✅ / `apex-core.md` 임계 🔲** | **① `_index/release.md` 분할 = 이미 완료** — 2026-08-26 **index-manager가 동시 세션에서 실행**했다(retrospective 예고와 독립·결론 동일). 연대순 절단: `_index/release-2024-2025.md` 신설(101줄 / 16,325 chars · Winter '24~Summer '25 v59~v64) + `release.md`는 최신 4세트만 유지(**30,178 → 25,783 chars**), 라우터 `00 SEARCH_INDEX.md` L31·32 2행 갱신. 이 항목의 원래 예고(Spring '27 초과 위험)는 **해소**. **② 잔여 = `_index/apex-core.md` 임계** — 2026-08-26 실측 **39,520 chars = 상한(~40k)의 98.8%.** 다음 apex 노트 1건이면 초과한다. 릴리즈 샤드와 달리 **연대 절단선이 없어** 절단 기준 선정이 선행 작업이다(제안: 언어/런타임 코어 vs 레퍼런스 카탈로그, 또는 `apex-core` / `apex-runtime`). **소유자 = index-manager**(retrospective는 권고만). ⚠️ 교훈: 샤드 상한은 **줄 수가 아니라 chars가 먼저 터진다**(release.md는 152줄/30k, apex-core는 상한의 98.8%인데도 줄 수는 여유) — `wc -l -c` 양쪽을 본다 | 🟠 P1 (index-manager 권고) | 🟡 부분 완료 (release ✅ / apex-core 🔲) | 2026-08-26 |
| W27-BL-4 | **텔레포니 메타데이터 타입 4종 미문서화 — GA 재확인 필수** | 릴리즈 노트 `rn_api_meta`가 신규 타입 `TelephonyProvider`·`SecondaryTelephonyProvider`·`TrustedTelephonyProvider`·`ScndTelephPrvdOtbdDtl`를 선언하나, v68.0 **PREVIEW** Metadata API 가이드에는 **전수 검색 0회**(대소문자 무시 + 부분 토큰 `telephonyprovider`/`scndteleph`/`otbddtl`/`byoc`). suffix·디렉터리·필드 표가 없어 **위키화 불가로 판단해 보류**. GA(비-PREVIEW) 가이드 게시 후 재확인 | ⏳ **게이트** (10월 GA 대기) | 🔲 대기 | 2026-08-25 |
| W27-BL-9 | **프리뷰 재확인 대상 (10월 GA 시)** | ①`rn_api_tooling_headers_rest_and_soap`가 **본문 `TBD`로 게시된 플레이스홀더** ②SOQL `FORMULA()`가 v68 REST 문서에 **0회** — Beta라서로 **추정**(단정 아님), SOQL 레퍼런스 v68 게시 후 확인 ③**안전필터로 미확보한 코드 샘플 3건**(`rn_apex_integration_tests` 샘플 클래스, `rn_apex_namespace_shadowing_managed_packages` 동적 SOQL 2건) ④**릴리즈 노트 자체 모순 3건**(guest PII 버킷 부재, `rn_api_soap_login`의 `When:` 부재, 재컴파일 API의 PDF 규칙 vs 예제 충돌) | ⏳ **게이트** (10월 GA 대기) | 🔲 대기 | 2026-08-25 |

> **다음 세션 착수 순서 권고:** `W27-BL-14`(허브·nav 동기화 — 완료분이 독자에게 안 보이는 상태라 최우선) → `W27-BL-12`(수 분·v68 PDF 로컬 확보·게시본 부정확) → `W27-BL-3`(남은 15리프·부재 선언 확정) → `W27-BL-13`(index-manager 샤드 분할 — apex-core 임계 동반) → `W27-BL-11`(단일 노트 파일럿) → `W27-BL-10`(30파일 노트별 판단, BL-11 판단 규칙 재사용). `W27-BL-4`·`W27-BL-9`는 **10월 GA 게시 전 착수 금지**.

#### 📋 브라우저 추출 레시피 — 실사고 3건으로 개정됨 (2026-08-25/26)

> 위 재추출 레시피를 **실제로 돌릴 때** 반드시 지킬 것. 세 항목 모두 이번 세션의 **실패에서 나왔다** — 다음 릴리즈 인제스트(W27-BL-3·Spring '27)가 여기서 먼저 읽도록 백로그에 둔다.

| # | 규칙 | 실사고 (Why) |
|---|---|---|
| R-1 | ⛔ **`read_network_requests`를 호출하지 않는다** | help.salesforce.com에서 호출하면 **Coveo JWT·GA 쿼리스트링이 컨텍스트에 유입** → **세션 전역 안전 분류기 발동** → 이후 브라우저·Bash 쓰기·서브에이전트 스폰이 **전부 차단**된다. 한 배치가 **9/166에서 사망**했다. 디버깅 목적이라도 부르지 않는다 |
| R-2 | **`navigate` + `javascript_tool`을 페이지당 하나의 `browser_batch`로 묶고, slice 오프셋을 스크립트에 직접 박는다** | 동시 실행 중 **단독 `javascript_tool` 호출이 실패**한다(앞선 navigate와 레이스). 배치로 묶으면 순서가 보장된다 |
| R-3 | **`browser_batch`는 5페이지가 상한** | 10페이지는 **타임아웃**. localhost sink로 우회하려 해도 **CSP로 차단**된다 |
| R-4 | **중단 시 산출물 최상단에 `⚠️ INCOMPLETE RUN — N of M` + 재개 지점을 명시한다** | 이 헤더 한 줄이 **157페이지 재작업을 막았다.** **편집 작업 중단에도 동일 원칙** — 이번에 `Clouds.md`가 커버리지 표↔섹션 내용 불일치 상태로 남았을 때 caution 블록에 *"커버리지 표보다 섹션 내용을 신뢰하라"* 고 적었고, **재개 에이전트가 그 블록을 지표로 삼아** 정확히 이어받았다 |

> R-1·R-2·R-3은 **도구 사용 규칙**이라 scout `절대 금지`에도 반영했다(AP-13 적용분). R-4는 **산출물 규약**이라 researcher 추출 계약에 반영했다.
...
| ~~AI-CLASSIC-2b~~ | ~~Einstein Discovery REST 잔여 심화~~ (로컬 PDF 전 챕터) | ✅ **완료 — 로컬 PDF 전수 (2026-07-19)** — 로컬 `bi_dev_guide_rest_sdd.pdf`(Ch1–4 전체) → **총 7 신규노트**. **엔드포인트 2**(개요·인증·예측 소비 흐름 / 리소스 엔드포인트 32개) + **표현형 5**(요청 Input 86종 / 응답 모델·필드·소스 52 / 응답 예측·잡·내러티브 65 / 응답 스토리·인사이트 38 / Enums 64). 지표/편향 표현형은 기존 `— 모델 품질·편향 지표 표현형` 노트로 위임(중복0). researcher 2명 dump(dual pdftotext·anomaly 플래그) → writer 5명 도메인 분할 전사. 로스터 커버리지 교차검증(누락 4건 실측→Asset History 2·Abstract 3 보충). 원문 anomaly/typo 전수 보존. 상호 링크 18건 승격. Einstein Discovery REST 계열 **총 8노트** 완결. **잔여(별도)**: Prediction Builder Scorecard·Forecasting 채널 매트릭스 = help.salesforce.com Tier 2(로컬PDF 밖·니치) → 필요 시 신규 항목. → [[WORK_BACKLOG_ARCHIVE]] | 7노트 완료 | ✅ 완료 | 2026-07-18 |
...
| AP-13 | **영역 분할 병렬 추출 — 크로스 영역 페이지는 전 파티션 공통 배포** (W27-1 발견) — 릴리즈 노트를 영역별 파티션으로 나눠 병렬 추출하면 **여러 영역에 걸친 사실이 어느 파티션에도 온전히 담기지 않는다.** 실사고: 릴리즈 노트 **루트 페이지(`salesforce_release_notes`)가 Clouds 파티션에만 배정**돼, Platform writer는 "Platform 섹션이 6개 영역을 흡수했다"는 **핵심 구조 변화의 근거를 볼 수 없었고** 자기 덤프에 있는 3개 영역만 서술했다 — **writer 판단은 옳았다**(근거 없는 주장을 쓰지 않음). 결함은 지시가 아니라 **근거 배분**이다. → 예방 규칙: **루트·랜딩·change-log 등 크로스 영역 페이지는 모든 파티션에 공통으로 제공**한다(scout 파티션 설계 + researcher 추출 계약 양쪽). **부수 교훈 2건:** ⓐ `pdftotext`가 `"available in API\nversion 68.0"`을 개행으로 쪼개 **확인된 신규 타입 3건 중 2건이 평범한 grep에서 미검출** → **버전 문장 검색 전 개행 평탄화(`tr '\n' ' '`)를 기본**으로 ⓑ **정규식·파일 크기 비교 산출물은 후보 목록이지 결론이 아니다** — 이번에 25건 후보 중 진짜 신규 3건, **오탐 10건**(enum·JSON 속성·줄바꿈 파편·산문·Salesforce 자체 오타) → 후보→확정 사이에 **개별 원문 확인 의무**. Why 첨부, additive(기존 규칙 약화 없음). **⭐ 적용 (2026-08-26 모드 B):** ⑴ `scout.md` **§8 신설** — 파티션 설계 시 *공통 배포 세트*(루트·랜딩·허브·change-log·New and Changed 색인)를 먼저 확정해 **모든** 파티션에 배포하고 소스 맵 `### 파티션 설계` 절에 명시, **귀속은 접두어가 아니라 주제로 판정**(`rn_it_*`가 Agentforce 대신 Clouds로 떨어진 W27-BL-2 실사고 반영), 버전 문장 grep 전 `tr '\n' ' '` 개행 평탄화, 후보→확정 사이 개별 원문 확인 의무(25후보→진짜 3·오탐 10). ⑵ `scout.md` **절대 금지 4줄 추가**(공통 배포 세트 누락 · 접두어 귀속 · 개행 미평탄화 부재 단정 · `read_network_requests` 호출). ⑶ `researcher.md` **추출 계약 2절 추가** — 파티션 추출 시 공통 페이지 dump 선두 배치 + `### 크로스 영역 공통 페이지` 절 의무, 중단 시 `⚠️ INCOMPLETE RUN — N of M` 헤더 + 재개 지점(157페이지 재작업을 막은 실증). 브라우저 도구 레시피 R-1~R-4는 활성 백로그 🟠 W27 섹션에 표로 보존 | `scout.md`, `researcher.md` | ✅ **완료 (2026-08-26)** | 2026-08-25 |
