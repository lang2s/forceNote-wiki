...
| ING-20 | omnichannel_supervisor (32) | Service | ⏸️ 보류(저우선) — 내용 확인 결과 "Omni Supervisor"는 **개발자 가이드가 아닌 어드민/슈퍼바이저 기능 가이드**(설정·사용 중심). 개발 위키 가치 낮음. ADMIN 갭 진행 시 함께 검토 |
...
| ING-30 | service_presence_administrators (124) | Service (Omni-Channel admin) | 🔲 대기 — dev guide(23p)는 ING-19로 **완료(2026-06-20)**. 잔여 = `_administrators` 124p **어드민 가이드**(설정·관리). ⚠️ Standard Omni-Channel EOL Summer '26이라 가치 판단 필요(Enhanced Omni-Channel 우선?). 진행 시 `Service(서비스)/OmniChannel(옴니채널)/`에 합류 |
