...
| Visualforce (레거시·성숙) | 2 (코어·테스트·보안 / 현대화·이관) | "**이미 전수 인제스트된 성숙 도메인**(18노트). 진단 결과 콘텐츠 갭 거의 0 — 유일한 진짜 갭은 **Aura엔 있고 VF엔 없던 이관 결정 노트**(구조적 비대칭). 나머지는 findability(흩어진 사실 잇기)" → 신규 1 + 보강 4 | (이번 커밋) |
...
### 남은 파일럿 (백로그 — 우선순위순, 2026-07-08 등록 / Flow·Aura·VF 완료 2026-07-11)

Integration·Apex·LWC·**Flow**·**Aura**·**Visualforce** 완료. 다음 도메인을 같은 방법론으로 진행한다(괄호=콘텐츠 노트 수).
...
| ~~–~~ | ~~**Visualforce(비주얼포스)**~~ | ~~중~~ | ✅ **완료(2026-07-11)** — 신규 1 + 보강 4. 이미 전수 인제스트된 성숙 도메인 → 갭 최소: VF→LWC 이관 결정 노트(Aura 대칭) + findability 보강 |
| 1 | **Security(보안)** Secure Coding (26) | 중 | 크로스커팅 위협 모델 — Apex/Security와 구분, 절차·체크리스트 층 |
| 2 | **DevOps(데브옵스)** (101) | 대 | DX/CI·CD/Metadata/Tooling — 규모 커서 서브클러스터 분할 필수 |
| 3 | **Admin(어드민)** (47) | 대 | 설정 how-to 위주 — 개념·레퍼런스 대비 절차 층 점검 |
| 4 | **Service(서비스)** (40) | 대 | Service Cloud — 데이터모델 레퍼런스 vs 구성 절차 |
| 5 | **Analytics(애널리틱스)** (21) · **sObject**(38, 태생 레퍼런스) · **Architecture(아키텍처)**(24) | 중 | 레퍼런스 성숙, 개념·절차 보강 위주 |
| 6 | 클라우드/산업: Commerce·Scheduler·FieldService·CPQ·SalesCloud·DataCloud·Agentforce·AgentSkills·ConnectREST | — | 도메인별 소규모 — 필요 시 묶어서 |
